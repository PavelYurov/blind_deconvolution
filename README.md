...
Данный проект посвящен исследованию методов восстановления изображений (Blind Image Deconvolution) без априорной информации о функции искажения (PSF) с интегрированной системой автоматической оптимизации гиперпараметров. Основное внимание уделяется разработке и сравнению алгоритмов, способных реконструировать исходное изображение с высокой априорной неопределенностью о функции искажения. Проект обеспечивает комплексное исследование алгоритмов с систематической оценкой качества восстановления и подбором оптимальных гиперпараметров.

### Цель исследования

Имплементация, сравнительный анализ предельных возможностей методов для восстановления изображений, искаженных различными типами размытия и шумов. Имплементацию алгоритмов, систематическая оценку качества восстановления и автоматический подбор оптимальных гиперпараметров. 

### Основные задачи
- **Исследование современных методов Blind Deconvolution, PSF estimation, Super-Resolution**
- **Реализация байесовских методов**: Робастный с тяжелыми хвостами, Разреженный вариационный байес
- **Имплементация вариационных и оптимизационных методов**: Сглаживающие априоры, Методы дробного порядка, L0‑регуляризация интенсивностей и градиентов 
- **Разработка тензорных методов Deconvolution и Super-Resolution**: Нелокальная низкоранговая тензорная аппроксимация, Многоуровневое иерархическое разложение
- **Создание интегрированной системы автоматической оптимизации гиперпараметров и многокритериального анализа (multiobjectivization)**: байесовская оптимизация (GP, TPE), эволюционные алгоритмы (NSGA‑II), случайный поиск; построение многомерных Парето‑фронтов для визуализации компромиссов между качеством восстановления и устойчивостью к искажениям. 
...
- **Методы восстановления**: классические, байесовские, вариационные, разреженные, тензорные
- **Обработка**: монохромные и цветные изображения (JPEG, BMP, PNG), пакетная обработка
- **Генерация реалистичных искажений**: расфокус, motion blur, B-spline траектории; гауссов, пуассонов, импульсный шум
- **Метрики качества**: PSNR, SSIM, устойчивость к шумам, скорость выполнения
- **Оптимизация**: байесовская (Tree-Structured Parzen Estimator, Gаussіаn Рrосеssеs), Gеnеtіс Аlgоrіthms, NSGA-II
