...
from ..pyWavelet import waveTools as pm
from .pyUtils import *
from .pyProx import *
...
def DecGMCA(V,M,n,Nx,Ny,Imax,epsilon,epsilonF,Ndim,wavelet,scale,mask,deconv,wname='starlet',thresStrtg=2,FTPlane=True,fc=1./16,logistic=False,postProc=0,postProcImax=50,Kend=3.0,Ksig=3.0,positivityS=False,positivityA=False):
    ...
    for i in np.arange(Imax):
        ...
        Shat = np.asarray(Shat)
        if Shat.ndim == 1:
            if Shat.size == P:
                Shat = Shat.reshape((1, P))
            else:
                Shat = np.resize(Shat, (n, P))
        elif Shat.ndim == 2 and Shat.shape[0] != n and Shat.shape[1] == n:
            Shat = Shat.T
        if Shat.shape[0] != n:
            Shat = np.resize(Shat, (n, P))
        Shat = Shat.astype(np.complex128, copy=False)
