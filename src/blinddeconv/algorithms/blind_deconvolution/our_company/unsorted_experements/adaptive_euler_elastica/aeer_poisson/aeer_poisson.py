...
# Импорты фреймворка (как в шаблоне)
import sys
...
class AEER_BD(DeconvolutionAlgorithm):
    def __init__(
        ...
        # --- Params (Poisson Model) ---
        lambda_val: float = 2000.0, # Вес Пуассоновской расходимости
        beta: float = 5.0,        # Штраф градиента ядра
        alpha: float = 0.05,
        iota: float = 5.0,
        delta: float = 1.0,
        
        # ADMM штрафы (r1, r2, r3, r4) - В новой статье их 4
        r1: float = 50.0,
        r2: float = 50.0,
        r3: float = 200.0, 
        r4: float = 10.0, # Штраф привязки g к Ku (из статьи - кратно 5)
        ...
        # Для Пуассоновской модели финальная деконволюция на ADMM ('admm') будет самой математически правильной,
        # так как она использует именно Пуассоновскую целевую функцию.
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        #     f = pad_image(f_orig, (kh, kw))
        # elif self.boundary_handling == 'edgetaper':
        ...
        u = f.copy()
        g = f.copy() # Инициализация новой переменной g
        ...
        lambda4 = np.zeros((H, W)) # Множитель для условия g = Ku
        ...
        for n in range(self.max_iter):
            ...
            # --- Update g (Poisson fidelity step) ---
            ...
            # Вычисление Ku для обновления lambda4
            F_k = psf2otf(k, (H, W))
        ...
        if self.final_deconv == 'wiener':
            ...
            # Точная неслепая деконволюция (Poisson Elastica) с фиксированным ядром
