...
# Absolute import — robust to deeply-nested package paths.
...
# ─────────────────────────────────────────────────────────────────────────────
# Default ``auto_mode='robust'`` config for the legacy branch
# (mirrors the kwargs that win in the user's notebook for colored / clean)
# ─────────────────────────────────────────────────────────────────────────────
...
# Threshold: psd_sigma_norm above which Robust beats Legacy on white
# Gaussian.  Tuned from test_robust_pmp results.
_HEAVY_SIGMA_DEFAULT = 0.015
# Impulse density above which Robust pre-pass is essential.
_IMPULSE_DENSITY_HEAVY = 0.01
# Pyatykh a_norm above which Poisson signature is unambiguous.
_POISSON_A_NORM = 1e-3


class PMP_BD_Merged(DeconvolutionAlgorithm):
    """Dispatcher: classify noise, then run the better PMP variant.

    Parameters
    ----------
    kernel_size : int
        Forwarded to the chosen inner algorithm.
    force : ``None`` | ``'robust'`` | ``'legacy'``
        Bypass the dispatcher.
    heavy_sigma_threshold : float
        ``psd_sigma_norm`` above which Robust is used for white Gaussian.
    impulse_density_heavy : float
        Impulse density above which Robust is forced.
    poisson_a_threshold : float
        Pyatykh ``a_norm`` above which Robust is forced (VST).
    robust_kwargs : dict
        Extra kwargs forwarded to ``PMP_BD_Robust`` constructor.
    legacy_kwargs : dict
        Extra kwargs *overriding* ``_LEGACY_DEFAULT_KWARGS`` and forwarded
        to ``PMP_BD`` constructor.
    verbose : bool
        Print routing decision.
    name : str
        Algorithm name reported by ``get_name()`` (default ``'PMP_BD_Merged'``).
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Filled after process() is called.
        self.last_branch: Optional[str] = None
        self.last_descriptor: Optional[dict] = None
        self.last_inner_alg: Any = None

    # ──────────────────────────────────────────────────────────────────
    # DeconvolutionAlgorithm contract
    # ──────────────────────────────────────────────────────────────────
    def change_param(self, param: Dict[str, Any]) -> None:
        """Bulk-update hyperparameters.

        Recognises the dispatcher's own knobs.  Other keys are routed by
        ``robust__`` / ``legacy__`` prefix; unprefixed unknown keys are
        forwarded to **both** kwarg dicts (the wrong-branch constructor
        will silently drop them via the inspect-based fallback in
        ``PMP_BD_Robust``).
        """
    ...
    def get_param(self) -> List[Tuple[str, Any]]:
        """Return tuples (name, value) for logging / saving."""
    ...
    def _classify(self, image: np.ndarray) -> Tuple[str, dict]:
        """Return ('robust'|'legacy', descriptor)."""
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """Classify noise → run the better branch.

        Times the *whole* dispatcher (analyse + inner alg) and stores it
        in ``self.timer`` so ``Processing.restorepipeline`` can log it.
        """
        t0 = time.time()
        ...
        # Forward callback if user installed one (e.g. for hyperparam
        # optimisation); inner alg may or may not honour it.
