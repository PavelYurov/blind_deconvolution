...
def coarse_kernel_est(
    ...
    **Latent image estimation:**
    ...
    # Threshold negative values and small positive noise, then normalise
    k[k < 0] = 0.0
    if k.max() > 0:
        k[k < k.max() * 0.05] = 0.0
    k_sum = k.sum()
...
def _solve_min_laplacian(boundary_image: np.ndarray) -> np.ndarray:
    ...
    # Boundary contribution: Laplacian of the boundary values
    f = np.zeros_like(boundary_image)
    # row boundaries
    f[1, 1:W-1]   += boundary_image[0, 1:W-1]
    f[H-2, 1:W-1] += boundary_image[H-1, 1:W-1]
    # col boundaries
    f[1:H-1, 1]   += boundary_image[1:H-1, 0]
    f[1:H-1, W-2] += boundary_image[1:H-1, W-1]
...
def wrap_boundary(img: np.ndarray, img_size: tuple) -> np.ndarray:
    ...
    ret = np.zeros((H_out, W_out), dtype=np.float64)
    alpha = 1
    ...
    # Assemble
    ret[:H, :W] = HG
    ret[:H, W:] = B[:, alpha:alpha + W_w]
    ret[H:, :W] = A[alpha:alpha + H_w, :]
    ret[H:, W:] = C[alpha:alpha + H_w, alpha:alpha + W_w]
