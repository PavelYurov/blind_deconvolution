...
def clean_kernel(k: np.ndarray, threshold_ratio: float = 20.0) -> np.ndarray:
    """
    Clean an estimated blur kernel by thresholding small values,
    keeping only the largest connected component, and re-normalising.

    Parameters
    ----------
    k               : (kh, kw) raw kernel estimate.
    threshold_ratio : values below max(k) / threshold_ratio are zeroed.
                      Default 20 (matches DCP convention).

    Returns
    -------
    k_clean : (kh, kw) cleaned kernel (non-negative, sums to 1).
    """
    k = k.copy()
    ...
    # 2. Threshold small positive values
    if k.max() > 0:
        k[k < k.max() / threshold_ratio] = 0.0

    # 3. Keep only the largest connected component (8-connectivity)
    if k.max() > 0:
        binary = (k > 0).astype(np.int32)
        labeled, n_components = label(binary,
                                      structure=np.ones((3, 3), dtype=np.int32))
        if n_components > 1:
            # Find the component with the largest total mass
            best_label = 0
            best_mass = 0.0
            for lbl in range(1, n_components + 1):
                mass = k[labeled == lbl].sum()
                if mass > best_mass:
                    best_mass = mass
                    best_label = lbl
            k[labeled != best_label] = 0.0
...
def center_kernel(k: np.ndarray) -> np.ndarray:
    """
    Shift the kernel so that its centre of mass is at the geometric centre.

    Uses circular shift (np.roll) which preserves energy.
    ...
    Returns
    -------
    k_centered : (kh, kw) centred kernel.
    """
    kh, kw = k.shape
    cy, cx = kh // 2, kw // 2
    ...
    com_y = (yy * k).sum() / total
    com_x = (xx * k).sum() / total

    # Shift to centre
    shift_y = int(round(cy - com_y))
    shift_x = int(round(cx - com_x))

    return np.roll(np.roll(k, shift_y, axis=0), shift_x, axis=1)
