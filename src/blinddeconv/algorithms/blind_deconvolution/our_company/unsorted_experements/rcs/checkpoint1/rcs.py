...
class RCS_BD(DeconvolutionAlgorithm):
    ...
    convergence      : float — convergence threshold for variational
                       inference (default 1e-4).
    max_iterations   : int — max iterations per scale (default 1000).
    ...
    init_prescision  : float — initial precision for parameters (default 1.0).
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.rescale_then_grad:
            ...
            mask_scale[NUM_SCALES - 1] = mask_all_patch.copy()
        ...
        if self.priors is not None:
            ...
            for s_idx in range(NUM_SCALES):
                priors_list.append({
                    'pi': np.ones((obs_imz, self.image_components)),
