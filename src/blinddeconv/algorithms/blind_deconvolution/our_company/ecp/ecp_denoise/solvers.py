...
def blind_deconv_main_BDF(blur_B, k, lambda_dark, lambda_grad, threshold, opts,
    ...
    Optional hooks (defaults None — preserve original behaviour):
      ...
      • progress_callback(event_dict) — called once per iteration after
        the kernel update with telemetry: ``{scale, iter, kernel,
        kernel_diff, lambda_dark, lambda_grad, sparsity}``.
    ...
    for _iter in range(xk_iter):
        ...
        if blind_denoise_fn is not None:
            try:
                ...
                # Never break the solver if the user-supplied denoiser
                # fails — silently fall back to the un-denoised S.
                pass
