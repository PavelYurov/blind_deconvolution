...
Также включает метод неслепой деконволюции из работы:
    D. Krishnan, R. Fergus: "Fast Image Deconvolution using
    Hyper-Laplacian Priors", NIPS 2009.

Содержит:
    - TV_denoising : предварительное шумоподавление с TV-регуляризацией (Split Bregman).
...
def Deblur_GL_CG_4(Y_b, k, W, we, max_iter):
    """
    Восстановление промежуточного структурного изображения (skeleton) 
    с использованием графового лапласиана и метода сопряженных градиентов.

    Решает задачу минимизации:
    ...
    ---------
    Y_b : ndarray
        Размытое изображение (с возможным дополнением границ).
    k : ndarray
        Текущая оценка ядра размытия.
    W : ndarray
        Массив весов графа размерности (h*w, 4).
    we : float
        Вес графового регуляризатора (параметр mu).
    max_iter : int
...
def kernel_solver_L2(Y, b, k_size, M, lambda_val):
    """
    Оценка ядра размытия в градиентной области методом сопряженных градиентов.
...
def bid_rgtv_c2f_cg(Y_b, k_estimate_size, show_intermediate=False):
    """
    Слепая деконволюция с использованием графовой регуляризации (RGTV) 
    в иерархическом подходе (от грубого масштаба к точному).
...
def Deconvolution_FHLP(y, kernel, lambda_val=2e3, alpha=0.5):
    ...
    kernel : ndarray
        Оцененное ядро размытия.
    lambda_val : float, по умолчанию 2e3
