...
class GBBID(DeconvolutionAlgorithm):
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # ── 2*. Sanitation cascade (experimental, shared module) ────────
        # When auto_mode='sanitation' the entire 2½–3¼ noise pipeline is
        # bypassed: the shared `_sanitation_cascade` produces an already-
        # clean image and a residual-σ descriptor.  Stages 4–5 (blind
        # kernel + non-blind) run normally on the cleaned image.
        ...
        if self.auto_mode == 'sanitation':
            ...
            _verbose = bool(getattr(self, 'verbose', False))
            sanitation_result = _sanitize(yg, verbose=_verbose)
            yg = sanitation_result.image_clean.astype(np.float64, copy=False)
            # Mirror to full-colour `y` so non-blind also benefits.
            if y.ndim == 3:
                # Keep colour channels independent; sanitise each channel
                # so chroma noise does not bleed back into the non-blind
                # restoration.  This is intentionally a per-channel pass:
                # cascade decisions on luminance may differ from chroma.
            ...
            if _verbose:
                print(f"[GBBID] sanitation: branch="
        ...
        # In sanitation mode the input is already cleaned; suppress
        # additional in-blind preprocess/pre_kernel denoisers that would
        # erode genuine signal detail.
