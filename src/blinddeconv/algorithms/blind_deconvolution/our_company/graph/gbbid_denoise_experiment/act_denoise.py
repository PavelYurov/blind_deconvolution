...
def _choose_num_scales(H, W):
    ...
    Mirrors the previous heuristic: ``ceil(log2(min(H,W))) - 2`` clamped
    to the range ``[2, 4]``.
    """
...
def _udct_pad_multiple(num_scales):
    ...
    Empirically, ``curvelets.numpy.UDCT`` rounds decimation ratios when
    a dimension is not divisible by ``2 ** (num_scales - 1)`` and silently
    produces large reconstruction error.  Padding to a multiple of this
    value restores perfect reconstruction.
    """
    return 1 << max(num_scales - 1, 0)


def _make_udct(H, W, num_scales=None):
    """Create a UDCT operator for an image of the given shape.

    The shape ``(H, W)`` MUST already be padded to multiples of
    ``_udct_pad_multiple(num_scales)`` -- otherwise the underlying UDCT
    fails silently with bad reconstruction.  See :func:`act_denoise` which
    handles padding for callers.
    """
...
def act_denoise(image, noise_var=None, threshold_setting='s'):
    ...
    # ── Pad to a UDCT-compatible shape ───────────────────────────────
    # UDCT silently breaks (huge reconstruction error) when a spatial
    # dimension is not divisible by 2**(num_scales-1).  This causes
    # severe block / banding artifacts in non-square or odd-sized
    # images.  Pad each dim up to the next multiple of that factor.
    num_scales = _choose_num_scales(H, W)
    ...
    if blind:
        # MAD on the finest scale.  For non-square images individual
        # wedges have very different aspect ratios, so a single wedge
        # is biased; aggregate MAD across all wedges of the finest
        # scale and take the median.
        mads = []
