...
class SI_BD(DeconvolutionAlgorithm):
    ...
    lambda_tv : float
        ...
        (used when use_tv=True).  Default 0.001.
    use_tv : bool
        If True, use TV-ADMM for non-blind deconvolution (recommended).
        If False, use the original multi-derivative Wiener solver.
        Default True.
    kernel_refine_iters : int
        Number of kernel re-estimation iterations at the finest scale
        after the multi-scale loop.  Removes the faint shadow/ringing.
        Default 3.
    """
    ...
    def __init__(
        ...
        self.use_tv = use_tv
        self.kernel_refine_iters = kernel_refine_iters
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(n_scales):
            ...
            # Clean kernel at each scale to prevent artifact accumulation
            k = clean_kernel(k, threshold_ratio=10.0)
        ...
        # ── 4. Kernel refinement at full resolution ──────────────────
        # Re-estimate kernel 2-3 times at the finest scale to sharpen it
        # and remove the faint "shadow" (low-amplitude ringing).
        ks_last = ks_list[-1]
        ksize_last = 2 * ks_last + 1
        kernel_gaus = fspecial_gaussian(ksize_last, self.edgetaper_sigma)

        Im_full = imresize(I, 1.0 / ratios[-1])  # finest scale
        min_h = min(Im_full.shape[0], I_latent.shape[0])
        min_w = min(Im_full.shape[1], I_latent.shape[1])
        Im_full = Im_full[:min_h, :min_w]

        for _ in range(self.kernel_refine_iters):
            # Pad + edgetaper
            Im_ref = np.pad(Im_full, ((ks_last, ks_last), (ks_last, ks_last)), mode='edge')
            Il_ref = np.pad(I_latent[:min_h, :min_w],
                           ((ks_last, ks_last), (ks_last, ks_last)), mode='edge')
            for _ in range(self.n_edgetaper):
                Im_ref = edgetaper(Im_ref, kernel_gaus)
                Il_ref = edgetaper(Il_ref, kernel_gaus)

            # Shock-filter the latent estimate for sharp edges
            I_sh_ref = perona_malik(Il_ref, self.pm_iter)
            I_sh_ref = shock_filter(I_sh_ref, self.sf_iter, self.sf_dt, self.sf_h)

            # Gradients
            Im_ref_x, Im_ref_y = gradient_circular(Im_ref)
            Ish_ref_x, Ish_ref_y = gradient_circular(I_sh_ref)

            # M + H masks
            M_ref, _ = m_compute(Im_ref_x, Im_ref_y, ks_last,
                                 is_first_scale=True, tau_r=0.0)
            H_ref, _ = h_compute(Ish_ref_x, Ish_ref_y, M_ref, ks_last,
                                 is_first_scale=True, tau_s=0.0)

            Is_ref_x = Ish_ref_x * H_ref
            Is_ref_y = Ish_ref_y * H_ref

            k, I_latent_ref = coarse_kernel_est(
                Is_ref_x, Is_ref_y, Im_ref_x, Im_ref_y, Im_ref,
                ksize_last, self.lambda_kernel, self.gamma,
            )
            k = clean_kernel(k, threshold_ratio=8.0)

            # Update latent for next iteration (crop padding)
            xr, yr = Im_ref.shape
            I_latent = I_latent_ref[ks_last:xr - ks_last, ks_last:yr - ks_last]

        # ── 5. Final kernel cleanup ────────────────────────────────────
        k = clean_kernel(k, threshold_ratio=8.0)
        k = center_kernel(k)

        # ── 6. Non-blind deconvolution ──────────────────────────────────
        # Use original (unpadded) image for the non-blind step
        # to avoid padding artifacts from the multi-scale loop.
        if self.use_tv:
            I_latent = tv_deconv(I, k, lambda_tv=self.lambda_tv)
        else:
            # Apply edgetaper to reduce boundary ringing
            ks_last = ks_list[-1]
            ksize_last = 2 * ks_last + 1
            kernel_gaus = fspecial_gaussian(ksize_last, self.edgetaper_sigma)
            I_et = I.copy()
            for _ in range(self.n_edgetaper):
                I_et = edgetaper(I_et, kernel_gaus)
            I_latent = multi_deriv_deconv(I_et, k, self.lambda_deconv)
    ...
    def get_param(self) -> List[Tuple[str, Any]]:
        return [
            ...
            ('lambda_tv', self.lambda_tv),
            ('use_tv', self.use_tv),
            ('kernel_refine_iters', self.kernel_refine_iters),
        ]
