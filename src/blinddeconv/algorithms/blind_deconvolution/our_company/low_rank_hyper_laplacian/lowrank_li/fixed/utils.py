...
def imresize(img: np.ndarray, target_size, method: str = 'bilinear') -> np.ndarray:
    ...
    Closer to MATLAB ``imresize`` than ``scipy.ndimage.zoom``:
      * uses cv2.INTER_AREA on downscale (anti-aliasing — critical for the
        coarse-to-fine pyramid; without it high-frequency aliasing corrupts
        gradient images and the kernel estimate, especially for non-trivial
        kernels);
      * uses cv2.INTER_LINEAR on upscale (bilinear, matches MATLAB).

    Falls back to ``scipy.ndimage.zoom`` if cv2 is not available.
    """
    ...
    if _cv2 is not None:
        downscale = (th < sh) or (tw < sw)
        if method == 'bilinear':
            interp = _cv2.INTER_AREA if downscale else _cv2.INTER_LINEAR
        else:  # 'bicubic'
            interp = _cv2.INTER_AREA if downscale else _cv2.INTER_CUBIC
        # cv2.resize takes (width, height)
        result = _cv2.resize(src, (tw, th), interpolation=interp)
        return result.astype(np.float64)

    # ── Fallback: scipy zoom (no anti-aliasing) ──────────────────────────
    order = 1 if method == 'bilinear' else 3
    zh = th / sh
    zw = tw / sw
    result = zoom(src, (zh, zw), order=order)
    rh, rw = result.shape
    if rh != th or rw != tw:
...
def _newton_w(v: np.ndarray, beta: float, alpha: float) -> np.ndarray:
    ...
    MATLAB also checks whether the zero solution is better:
        z = beta/2 * v^2;  f = |x|^alpha + beta/2*(x-v)^2;
        w = (f < z) .* x;
    """
    ...
    # Check whether zero solution is better (matches MATLAB newton_w)
    z = beta / 2.0 * v ** 2
