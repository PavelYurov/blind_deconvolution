...
def blinddeconv_new2_cry(y1, y2, x1, x2, lambda_, sigma, k,
                         ...
                         threshold, L2norm, verbose=False):
    """
...
def multiscaled_cry(y, K, params, verbose=False):
    """
    ...
    multiscale = params.get('multiscale', True)
    kernel_upsample = params.get('kernel_upsample', 'bilinear')  # 'bilinear' | 'nearest'
    sharpen_after_upsample = params.get('sharpen_after_upsample', False)
    ...
    if multiscale:
        ...
        # Single-scale: estimate directly at the final kernel size.
        minscale = K
        scales = [K]

    # ── Initial kernel ─────────────────────────────────────────────────────────
    init_kernel = params.get('init_kernel', 'line')
    if init_kernel == 'uniform':
        # Isotropic prior — no horizontal motion bias
        k = np.ones((minscale, minscale), dtype=np.float64) / (minscale * minscale)
    else:
        # MATLAB default: horizontal 2-pixel line
    ...
    for idx, Ki in enumerate(scales):
        ...
        if idx != 0:
            k = imresize(k, (Ki, Ki), kernel_upsample)
            # Optionally re-sharpen after smoothing introduced by upsampling.
            # Bilinear upsampling turns a sharp line into a bell along the
            # motion direction; without this fix, optimizek + mu-proximity
            # tend to lock the kernel into that bell shape.
            if sharpen_after_upsample:
                kt = params.get('threshold', 0.0)
                if kt > 0:
                    k[k < k.max() * kt] = 0.0
                ks = k.sum()
                if ks > 0:
    ...
    # Replicate padding instead of MATLAB edgetaper.
    # We pad by kernel size (2*bhs) on each side so the FFT-based deconvolution
    # has enough boundary to absorb circular-convolution artefacts; the centre
    # of the padded region is the original image, so cropping by 2*bhs returns
    # the original-size restored image.
    pad = 2 * bhs
    ypad = np.pad(y, pad, mode='edge')

    tmp = fast_deconv_bregman(ypad, k, nb_lambda, nb_alpha)
    x = tmp[pad:tmp.shape[0] - pad, pad:tmp.shape[1] - pad]
