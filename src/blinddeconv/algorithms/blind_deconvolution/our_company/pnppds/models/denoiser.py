...
# Добавляем pnppds/ в sys.path, чтобы torch.load (pickle) мог найти модуль 'models'
_pnppds_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _pnppds_dir not in sys.path:
    sys.path.insert(0, _pnppds_dir)
...
def load_checkpoint(model, file_name):
    # Добавлен аргумент weights_only=False
