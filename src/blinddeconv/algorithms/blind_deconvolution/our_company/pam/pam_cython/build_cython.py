"""
build_cython.py  —  Build all .pyx modules in pam_cython.

Usage (from any directory):
    python  <path-to>/pam_cython/build_cython.py
...
# Рядом лежит пакет pam_cython_pyd, из которого реально импортируется
# скомпилированный код.  Кладём туда же, чтобы новый билд сразу применялся.
INSTALL_DIR = PKG_DIR.parent / "pam_cython_pyd"
...
def build_extensions(c_files):
    ...
    tmp_root = Path(tempfile.gettempdir()) / "cython_pam"
