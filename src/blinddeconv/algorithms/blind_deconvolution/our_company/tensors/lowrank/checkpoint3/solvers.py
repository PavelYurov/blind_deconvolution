...
3. **Low-rank kernel regularisation** — Iteratively Reweighted Nuclear
   ...
   Applicable to the 2D kernel matrix in the single-image setting.
...
def low_rank_regularization(
    ...
    *weighted* nuclear-norm problem at each step whose solution is
    ...
    .. note::
        ...
        This regulariser operates on the 2D kernel matrix and is
        appropriate for the **single-image** setting.  For multi-image
        blind deconvolution, Algorithm 8 of [6] (iALM RPCA) should be
        used instead, decomposing the vectorised kernel stack.
