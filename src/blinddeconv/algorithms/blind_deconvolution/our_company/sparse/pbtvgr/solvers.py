...
def solve_u_subproblem(g: np.ndarray, h: np.ndarray, o: np.ndarray, 
    ...
    Eq. (16) - (18) in Dong et al.
    
    Objective: (lambda/2) * KL(u, g) + (beta/2) ||h*o - u||^2
    Solution: Positive root of quadratic equation.
    """
    H, W = g.shape
    ...
    # Eq. (17): 2*beta*u^2 + [lambda - 2*beta*ho]*u - lambda*g = 0
...
def solve_h_subproblem(u: np.ndarray, o: np.ndarray, h_prev: np.ndarray, 
    ...
    CRITICAL IMPLEMENTATION DETAIL (Eq. 13):
    ...
    # 1. Prepare "blurred observations" for h estimation
    ...
    for l in range(l_max):
        ...
        def matvec(v_flat):
            ...
            # 1. Fidelity term: beta * (conv(g_x)^T conv(g_x) + ...) v
            F_v = psf2otf(v, (H, W))
            ...
            # 2. Regularization term: mu * D^T W D v
        ...
        # Constraints (Algorithm 2, Step 5)
...
def solve_nonblind_tv(g: np.ndarray, h: np.ndarray, o_init: np.ndarray, 
    ...
    Uses variable splitting (Wang et al. 2008).
