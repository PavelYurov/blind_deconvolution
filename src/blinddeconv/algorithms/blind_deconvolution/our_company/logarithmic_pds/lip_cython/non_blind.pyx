...
def _interpolate_library(blurred, kernel, alpha, alpha_n, lam_map, lam_library):
    ...
    for idx in range(len(lam_library)):
        if not sat:
            I_library[idx] = _deconv_with_padding(blurred, kernel, alpha, alpha_n, lam_library[idx])
        else:
    ...
    for idx in range(len(lam_library)):
        I_lib_arr[idx] = I_library[idx]

    I_opt = np.zeros((M, N), dtype=np.float64)
    
    # Вызов быстрой C-функции
    _do_interpolate(np.ascontiguousarray(lam_map, dtype=np.float64), 
                    np.ascontiguousarray(lam_library, dtype=np.float64), 
                    np.ascontiguousarray(raw_idx, dtype=np.int32), 
                    np.ascontiguousarray(I_lib_arr, dtype=np.float64), 
                    I_opt)

    return np.clip(I_opt, 0.0, 1.0)
