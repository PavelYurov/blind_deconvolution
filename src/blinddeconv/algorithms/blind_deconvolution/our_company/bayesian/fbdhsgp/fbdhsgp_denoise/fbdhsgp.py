...
При auto_mode='robust' применяется единая упрощенная политика адаптации к шуму:
    1. Всегда запускается обнаружение импульсного шума (автоматически).
    2. Всегда оценивается уровень шума (автоматически через 'pca').
...
firls_lambda и т.д.) оркестратором не изменяются.
...
class FBDHSGP(DeconvolutionAlgorithm):
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        f_raw = y.copy()  # Нетронутая копия для ветви pre_nonblind
