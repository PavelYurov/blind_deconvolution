...
Вспомогательные функции для алгоритма FBDHSGP.
...
Содержит:
    ...
    - imresize_bilinear: Билинейная интерполяция с возможностью использования антиалиасинга.
...
# Используется scikit-image для изменения размера с предварительным 
# антиалиасинговым низкочастотным фильтром. Если библиотека недоступна, 
# происходит откат к OpenCV.
try:  # pragma: no cover
...
def pad_replicate(x: np.ndarray, pad_h: int, pad_w: int) -> np.ndarray:
    """Дополняет границы изображения путем копирования крайних пикселей."""
...
def fspecial_gaussian(shape: Sequence[int], sigma: float) -> np.ndarray:
    """
    Создает двумерный гауссовский фильтр.
...
def imresize_bilinear(img: np.ndarray, target_shape: Sequence[int]) -> np.ndarray:
    ...
    При значительном уменьшении масштаба в пирамиде важно применять
    предварительный низкочастотный фильтр для устранения алиасинга.
    Без антиалиасинга высокочастотный шум на грубых масштабах может 
    привести к смещению оценки ядра и образованию артефактов в финальном 
    изображении. Если доступна библиотека skimage, она автоматически 
    применяет такой фильтр при даунсэмплинге.
    """
...
def getindex(n1: int, n2: int, hks1: int, hks2: int):
    ...
    для применения H и H^T с неопределенными граничными условиями.
