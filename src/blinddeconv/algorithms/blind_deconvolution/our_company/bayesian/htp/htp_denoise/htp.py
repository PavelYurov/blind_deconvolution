...
Содержание алгоритма:
    ...
    2. Опциональная предварительная обработка (подавление импульсного шума 
       и оценка дисперсии шума).
    3. Построение пирамиды изображений от грубого к точному масштабу для 
       центральной области интереса (ROI).
    4. Многомасштабная чередующаяся MAP-оценка для скрытого изображения u 
       и функции рассеяния точки (ФРТ) h. Решается через полуквадратичное 
       расщепление и итерации Брегмана в Фурье-области.
    5. Финальная неслепая деконволюция (несколько методов на выбор: классическая,
       с подавлением краевых артефактов или FIRLS-UBC).
...
class HTP_BD(DeconvolutionAlgorithm):
    ...
    -------------------
    kernel_size : int
        Пространственный размер неизвестной функции рассеяния точки (квадрат). 
        По умолчанию 31.
    Lp : float
        Экспонента Lp-нормы для априорного распределения на градиенты во время 
        оценки ФРТ (0 < p <= 1). По умолчанию 0.3 (тяжелые хвосты).
    gamma : float
        Вес согласования данных при оценке ФРТ. По умолчанию 1e2.
    alpha_u : float
        Относительный вес априорного распределения изображения. По умолчанию 1e-2.
    beta_u : float
        Относительный вес расщепления Брегмана для изображения. По умолчанию 1e0.
    alpha_h : float
        Относительный вес L1-априорного распределения ФРТ. По умолчанию 1e1.
    beta_h : float
        Относительный вес расщепления для ФРТ. По умолчанию 1e4.
    ...
    Lp_nonblind : float
        Экспонента Lp для финального неслепого шага. По умолчанию 1.0 (TV-подобная).
    maxiter : int
        Внешние чередующиеся итерации на масштабе. По умолчанию 10.
    maxiter_u : int
        Внутренние итерации оценки изображения u. По умолчанию 10.
    maxiter_h : int
        Внутренние итерации оценки ФРТ h. По умолчанию 10.
    ccreltol : float
        Относительный допуск остановки. По умолчанию 1e-3.
    MSlevels : int
        Количество масштабов пирамиды. По умолчанию 4.
    maxROIsize : tuple[int, int]
        Область интереса для оценки ядра. По умолчанию (1024, 1024).
    verbose : int
        Уровень вывода. По умолчанию 0 (без вывода).
    kernel_flip : str
        Опция отражения/поворота финального ядра ('none', 'lr', 'ud', 'rot180').
    auto_recenter : bool
        Если True, применяется пространственное центрирование итогового ядра.
    recenter_mode : str
        Режим центрирования: 'centroid', 'peak', 'masscentroid'.
    kernel_thresh : float
    ...
    Параметры шумоподавления (расширения)
    -------------------------------------
    pre_pyramid, pre_kernel, pre_nonblind : str или None
        Выбор метода шумоподавления на соответствующих стадиях пайплайна 
        (см. denoisers.py).
    noise_estimation : str
        Метод оценки шума: 'none', 'chen' или 'pyatykh'.
    impulse_preprocess : str
        'auto' включает автоматическое обнаружение и удаление импульсного шума.
    auto_mode : str
        'auto' автоматически настраивает методы и параметры шумоподавления 
        на основе оцененного шума.
    nonblind_method : str
        Выбор алгоритма финальной неслепой деконволюции ('fft_cg_sr_al', 
        'ringing_removal' или 'firls').
    """
    ...
    def __init__(
        ...
        if kernel_flip not in ('none', 'lr', 'ud', 'rot180'):
            raise ValueError(f"Недопустимое значение kernel_flip: {kernel_flip!r}")
        self.kernel_flip = kernel_flip

        if recenter_mode not in ('centroid', 'peak', 'masscentroid'):
            raise ValueError(f"Недопустимый recenter_mode: {recenter_mode!r}")
        ...
        if noise_estimation not in ('none', 'chen', 'pyatykh'):
            raise ValueError(f"Недопустимый метод оценки шума: {noise_estimation!r}")
        ...
        # --- Предобработка импульсного шума ---
        if impulse_preprocess not in ('none', 'auto'):
            raise ValueError(f"Недопустимый impulse_preprocess: {impulse_preprocess!r}")
        ...
        if auto_mode not in ('off', 'auto'):
            raise ValueError(f"Недопустимый auto_mode: {auto_mode!r}")
        ...
        if nonblind_method not in ('fft_cg_sr_al', 'ringing_removal', 'firls'):
            raise ValueError(f"Недопустимый nonblind_method: {nonblind_method!r}")
    ...
    def _auto_denoiser_config(noise_info: Dict[str, Any]) -> Dict[str, Any]:
        """
        Сопоставление оцененной статистики шума с конфигурацией хуков шумоподавления.
        Определяет эвристики для пуассоновского и гауссовского шума.
        Параметры алгоритма деконволюции при этом не затрагиваются.
        """
    ...
    def _default_params_for(
        ...
        """
        Возвращает параметры по умолчанию для методов шумоподавления 
        на основе оцененного стандартного отклонения (sigma).
        """
    ...
    def _recenter_kernel_and_image(
        ...
        изображения U для сохранения инвариантности свертки.
        Используется ограничивающая рамка (bounding box) порогового 
        значения ядра для робастного определения центра.
        """
        kh, kw = H.shape
        cy_int = kh // 2
        cx_int = kw // 2
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # --- 1a. Предварительная обработка импульсного шума ---
        ...
        if self.impulse_preprocess == 'auto':
            ...
            if y.ndim == 3:
                ...
                y = y_clean
            if self.verbose:
                print(f'[HTP_BD] impulse_preprocess=auto -> {self.impulse_info}')

        # --- 1b. Опциональная оценка шума ---
        ...
        # --- 1c. Автоматическая настройка (auto_mode) ---
        ...
        if self.auto_mode == 'auto' and self.noise_info is not None:
            ...
            # --- Выбор варианта неслепой деконволюции ---
                ...
                if sigma < 0.01:
                    ...
                    if self.verbose:
                        print(f'[HTP_BD] auto_mode -> nonblind_method='
                              f'firls (sigma={sigma:.3f}, чистое изображение)')
                else:
        ...
        # --- 3. Автоматическое центрирование ядра ---
