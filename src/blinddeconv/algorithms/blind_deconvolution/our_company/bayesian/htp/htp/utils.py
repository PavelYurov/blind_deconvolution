...
def bwmorph_clean(BW: np.ndarray) -> np.ndarray:
    ...
    isolated = counts == 1
    isolated[0] = False  # Фон не считается
    out = BW.copy()
...
def center_psf(H: np.ndarray, thresh: float) -> np.ndarray:
    ...
    # Вычисление сдвига для центрирования по ограничивающей рамке
    val = (L + R + 3 - hsize) / 2.0
    ...
    # Центрирование центра масс с точностью до пикселя
    s = out.sum()
    if s == 0:
        return out
    kh, kw = out.shape
...
def setup_lp_prior(q: float, alpha: float, beta: float) -> Callable[
    ...
    регуляризации в соответствии с Уравнением 3 из [1].

    Априорное распределение моделируется как:
        phi(s) = alpha * |s|^q     при |s| > u_star
        phi(s) = (beta/2) * s^2    при |s| <= u_star

    При 0 < q < 1 пороги рассчитываются численно. Возвращает функцию 
    для применения порога к градиентам изображения.
    """
    if q == 1.0:
        v_star = 0.0
        u_star = alpha / beta
...
def _kernel_cubic(x: np.ndarray) -> np.ndarray:
    """Ядро бикубической интерполяции (a = -0.5)."""
    absx = np.abs(x)
    absx2 = absx * absx
    absx3 = absx2 * absx
...
def _contributions(in_length: int, out_length: int, scale: float,
    ...
    При масштабировании < 1.0 ядро расширяется, что обеспечивает 
    эффект антиалиасинга (сглаживания высокочастотного шума).
    """
...
def _resize_along_dim(A: np.ndarray, dim: int,
    ...
    """Применение одномерной интерполяции вдоль указанной оси."""
...
def imresize(A: np.ndarray, scale, method: str = 'bicubic') -> np.ndarray:
    """
    Двумерная интерполяция изображения с поддержкой антиалиасинга при уменьшении.
    """
    A = np.asarray(A, dtype=np.float64)
    ...
    # Интерполяция выполняется вдоль оси с меньшим масштабом в первую очередь
...
# --- Сглаживание краев изображения (Edgetaper) ---
def edgetaper(I: np.ndarray, PSF: np.ndarray) -> np.ndarray:
    """
    Сглаживание границ изображения с использованием автокорреляции ФРТ 
    для предотвращения краевых эффектов (ringing) при БПФ.
    """
...
def _edgetaper_alpha_1d(proj: np.ndarray, N: int) -> np.ndarray:
    """Вычисление одномерных весов сглаживания на основе проекции ФРТ."""
    if N <= 1:
...
def _psf2otf_centered(PSF: np.ndarray, shape: Tuple[int, int]) -> np.ndarray:
    """Конвертация ФРТ в ОТФ с циклическим сдвигом центра в начало координат."""
    ph, pw = PSF.shape
    H, W = shape
