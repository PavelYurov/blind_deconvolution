...
Вспомогательные функции для алгоритма слепой деконволюции.

Функциональность:
    - Нормализация и денормализация интенсивностей.
    - Выделение области интереса.
    - Линейное масштабирование контраста.
    - Очистка изолированных пикселей маски.
    - Центрирование функции рассеяния точки.
    - Вычисление инвариантной к сдвигу среднеквадратичной ошибки (MSE).
    - Быстрое преобразование Фурье с дополнением нулями.
    - Интерполяция изображений с поддержкой эффекта антиалиасинга.
    - Определение оператора полуквадратичного сжатия для Lp-регуляризации.
    - Сглаживание границ для предотвращения краевых эффектов (ringing) при БПФ.
...
def simpnormimg(G: np.ndarray) -> Tuple[np.ndarray, float, float]:
    """Нормализация интенсивностей входного изображения в диапазон [0, 1]."""
...
def mat2gray(A: np.ndarray) -> np.ndarray:
    """Линейное масштабирование значений массива к диапазону [0, 1]."""
...
# --- Морфологическая обработка и области ---

def get_roi(G: np.ndarray, win: Tuple[int, int]) -> np.ndarray:
    """Выделение центрального окна изображения."""
    G = np.asarray(G)
...
def bwmorph_clean(BW: np.ndarray) -> np.ndarray:
    """Удаление изолированных активных пикселей из бинарной маски."""
...
def center_psf(H: np.ndarray, thresh: float) -> np.ndarray:
    """
    Пороговое отсечение и центрирование ФРТ в пределах опорного окна.
    
    В отличие от стандартного линейного масштабирования контраста, которое 
    может вносить постоянное смещение (DC bias) и искажать сложные формы ядер, 
    здесь применяется отсечение по нулю с сохранением относительных амплитуд.
    Интегральное выравнивание центра масс выполняется для предотвращения 
    глобального смещения.
    """
...
def calculate_mse(h: np.ndarray, hs: np.ndarray) -> float:
    """Вычисление инвариантной к сдвигу среднеквадратичной ошибки (MSE) между оцененной и истинной ФРТ."""
    h = np.asarray(h, dtype=np.float64)
...
def fft2_pad(X: np.ndarray, M: int, N: int) -> np.ndarray:
    """Двумерное быстрое преобразование Фурье с предварительным дополнением нулями."""
    return np.fft.fft2(X, s=(M, N))


# --- Операторы регуляризации ---

def setup_lp_prior(q: float, alpha: float, beta: float) -> Callable[[np.ndarray, np.ndarray], np.ndarray]:
    """
    Построение оператора полуквадратичного сжатия (shrinkage) для Lp-регуляризации.
    При 0 < q < 1 пороговые значения рассчитываются численно.
    """
    if q == 1.0:
        v_star = 0.0
        u_star = alpha / beta
...
def _contributions(in_length: int, out_length: int, scale: float,
    ...
    """
    Вычисление весов и индексов для одномерной интерполяции. 
    При масштабировании < 1.0 ядро расширяется, что обеспечивает эффект антиалиасинга.
    """
...
def imresize(A: np.ndarray, scale, method: str = 'bicubic') -> np.ndarray:
    """Двумерная интерполяция изображения с эффектом антиалиасинга при уменьшении масштаба."""
    A = np.asarray(A, dtype=np.float64)
...
def edgetaper(I: np.ndarray, PSF: np.ndarray) -> np.ndarray:
    """Сглаживание границ для устранения краевых эффектов (ringing) перед вычислением БПФ."""
