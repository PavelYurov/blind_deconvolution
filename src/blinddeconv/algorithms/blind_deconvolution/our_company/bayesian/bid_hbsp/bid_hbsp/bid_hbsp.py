...
Поддерживает два режима работы:
    ...
    'filter_space' (по умолчанию, формулировка из статьи):
        Декомпозиция на N=2 независимых задач для отфильтрованных изображений
        (Уравнения 17-18 из Castro-Macias et al. 2024).

    'image_space' (альтернативный режим):
        Оценка x как единого изображения методом сопряженных градиентов (CG) 
        с априорным распределением D^T Gamma D, затем вычисление градиентов 
        для оценки ядра. Оставлен для сравнения.
...
# --- Вспомогательные функции пирамиды ---
...
class BID_HBSP(DeconvolutionAlgorithm):
    ...
    kernel_shape : (kh, kw)
        Размер ядра.
    hs_scale : float
        Параметр масштаба b распределения гиперболического секанса. Меньше -> сильнее разреженность.
    noise_sigma : float
        Начальное стандартное отклонение шума. Точность beta_0 = 1/sigma^2.
    max_iter : int
    cg_iter, cg_tol : параметры метода сопряженных градиентов (CG).
    ...
    boundary_mode : 'none' | 'edgetaper' | 'edgetaper_iter' | 'padding'
        Обработка краевых эффектов круговой свертки FFT.
        'padding' - дополнение краев перед CG (рекомендуется).
    jacobi_mode : 'scalar' | 'perpixel'
        Аппроксимация дисперсии для диагонали (H^T H).
    center_kernel : bool
        Центрировать ядро после каждого масштаба (предотвращает дрейф).
    beta_update : bool
    beta_n_factor : float
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """Запуск полного пайплайна BID-HBSP."""
        ...
        if self.verbose:
            print(f"[{self.name}] {num_scales} масштабов, "
                  ...
                  f"режим={self.solver_mode}, ядро={self.kernel_solver}, "
        ...
        for s in range(num_scales - 1, -1, -1):
            ...
            # Вызов соответствующего решателя
            ...
            # Центрирование ядра для предотвращения дрейфа
        ...
        # --- 4. Финальная неслепая деконволюция ---
    ...
    # --- Решатель в пространстве изображения (альтернативный) ---
    def _run_image_space(self, y_work, y_level, h, beta, b,
                         kernel_shape, lambda_h, s, num_scales):
        """Метод сопряженных градиентов для x, затем оценка ядра через градиенты."""
        kh, kw = kernel_shape
        H_img, W_img = y_level.shape
        ...
        for it in range(self.max_iter):
            ...
            # Изображение наблюдения для CG (с паддингом или без)
            if use_padding:
                y_cg = y_pad
            ...
            # (a) Вычисление весов на основе градиентов текущего x
            gamma_x, gamma_y = compute_hs_weights(
                forward_diff_x(x_est), forward_diff_y(x_est),
                sigma_sq, b)

            # (b) Метод сопряженных градиентов в пространстве изображения
            ...
            # (c) Оценка ядра (использует оригинальные размеры y и x)
            ...
            # (e) Уменьшение параметра регуляризации ядра
    ...
    def _run_filter_space(self, y_work, y_level, h, beta, alpha,
                          kernel_shape, lambda_h, s, num_scales):
        """Пространство фильтров: независимые CG + оценка ядра QP (статья 2024)."""
        kh, kw = kernel_shape
        H_img, W_img = y_level.shape
        ...
        for it in range(self.max_iter):
            ...
            # (a) Вычисление весов для каждого фильтра
            theta = [
                compute_hs_weights_scalar(
            ...
            # (b) Метод сопряженных градиентов для каждого отфильтрованного изображения
            for n in range(N_FILT):
            ...
            # Решение в частотной области, затем использование невязки
            if self.beta_update:
