...
def psf2otf(psf: np.ndarray, shape: Tuple[int, int]) -> np.ndarray:
    ...
    чтобы центр ядра находился по индексу (0, 0) перед двумерным дискретным преобразованием Фурье.
...
def otf2psf(otf: np.ndarray, kernel_shape: Tuple[int, int]) -> np.ndarray:
    """Восстанавливает пространственную PSF из ее OTF с помощью обратного БПФ и обрезки.
...
def compute_hs_weights_scalar(
    ...
    В вариационной формулировке в пространстве фильтров априорная модель размещается непосредственно
    на пикселях отфильтрованного изображения x_n = F_n * x, поэтому обновление весов 
...
def threshold_kernel(
    ...
    """Обнуляет малые значения ядра (для поддержания разреженности) с последующей нормализацией.
