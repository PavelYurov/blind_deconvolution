...
Функции-решатели для алгоритма BID-HBSP.
...
Содержит:
    ...
    - Упрощенную оценку ядра с помощью фильтра Винера в частотной области (solve_kernel_fourier_filterspace, solve_kernel_fourier)
...
def solve_filtered_image_cg(
    ...
    max_cg_iter, cg_tol : параметры остановки CG.
...
def solve_kernel_fourier_filterspace(
    ...
    поправкой на неопределенность VB. Краевые строки и столбцы каждого
...
def solve_kernel_qp_filterspace(
    ...
    do_threshold : bool
        Обнулить элементы ядра ниже threshold_ratio * max(h) перед нормализацией.
...
def update_noise_precision(y: np.ndarray, h: np.ndarray, x: np.ndarray, beta_prev: float, damping: float = 0.5) -> float:
    """Обновление точности аддитивного шума (параметра beta)."""
    H, W = y.shape
    N = float(H * W)
    F_h = psf2otf(h, (H, W))
