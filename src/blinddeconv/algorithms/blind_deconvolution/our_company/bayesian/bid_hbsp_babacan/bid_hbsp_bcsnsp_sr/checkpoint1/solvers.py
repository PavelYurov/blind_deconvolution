...
Contains:
  - All solvers from the original BID-HBSP (solve_image_cg,
    solve_kernel_fourier, update_noise_precision, update_hs_weights,
    final_deconvolution).
  - A new ``sr_refine_step`` that runs a lightweight BCSNSP-SR pass
    with the current kernel estimate to periodically re-sharpen the
    image estimate inside the EM loop.
...
# ═════════════════════════════════════════════════════════════════════════════
#  NEW: Periodic SR refinement step
# ═════════════════════════════════════════════════════════════════════════════
...
def sr_refine_step(
    ...
    """Lightweight BCSNSP-SR pass to re-sharpen x during the EM loop.

    Called periodically (every ``sr_refine_every`` EM iterations) with the
    **current** kernel estimate h.  Compared to the initial SR pass, this
    uses fewer iterations since the kernel is already partially converged.

    The rationale: the EM image solver (CG with HS prior) can drift into
    a local minimum.  A short SR pass with the complementary TV+SAR prior
    acts as a "reset" that pulls x back toward the data manifold while
    preserving the kernel estimate h.

    Parameters
    ----------
    y          : (H, W) blurred observation.
    h          : (kh, kw) current kernel estimate.
    x_current  : (H, W) current image estimate (used as warm-start hint
                 via normalisation — the SR solver has its own initialisation).
    L          : pseudo-frame count.
    max_shift  : max sub-pixel shift.
    sr_maxit   : SR iterations (keep small, 3-8).
    lambda_prior : TV/SAR trade-off.
    pcg_thr    : PCG tolerance.
    pcg_maxit  : PCG max iterations.
    pcg_minit  : PCG min iterations.
    verbose    : print SR progress.
    seed       : random seed.

    Returns
    -------
    x_refined : (H, W) re-sharpened image, float64, [0, 1].
    """
    ...
    x_refined = x_vec.reshape(M, N, order="F")
