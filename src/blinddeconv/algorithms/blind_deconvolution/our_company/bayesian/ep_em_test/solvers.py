...
def solve_kernel_pgd(
    ...
    F_x = fft2(x)
    F_y = fft2(y)
    # Gradient of data fidelity: 2 * X^T (X h - y)
    # In freq: 2 * conj(F_x) * (F_x * F_h - F_y) = 2 * (|F_x|^2 F_h - conj(F_x)F_y)
    F_xx = np.abs(F_x)**2
    F_xy = np.conj(F_x) * F_y
    
    # Lipschitz constant approximation
    max_eig_xx = np.max(F_xx)
    # D_x is usually symmetric, use eigvalsh
    try:
        max_eig_Dx = np.linalg.eigvalsh(D_x).max()
    except:
        max_eig_Dx = 1.0 # Fallback
    ...
    for _ in range(inner_iter):
        ...
        h_old = h.copy() # Store previous 'h' (not y_mom)
        
        # Use momentum variable for gradient
        curr_h = y_mom
        ...
        # IMPORTANT: fftshift logic.
        # ifft2 result has (0,0) at index [0,0]. 
        # We need to shift it so the center is at [H//2, W//2] to crop easily, 
        # OR just roll it to match the kernel support centered at [0,0] in kernel space.
        # The easiest way standard in deconv:
