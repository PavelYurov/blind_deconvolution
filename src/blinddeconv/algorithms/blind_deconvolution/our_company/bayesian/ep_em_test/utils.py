...
def edgetaper(img: np.ndarray, kernel: np.ndarray) -> np.ndarray:
    ...
    """
    alpha = 0.5 # Tapering factor
    H, W = img.shape
    kh, kw = kernel.shape
    
    # Auto-correlation of the projection of the kernel
    # Simplify: just create a window based on kernel size
    # Create a weighting mask
    
    # 1. Create a 1D Hanning-like window for rows and cols
    # Size depends on kernel size
    pad_h = kh // 2
    pad_w = kw // 2
    ...
    # Simple implementation: blend original image with its blurred version at borders
    blurred = fftconvolve(img, kernel, mode='same')
    
    # Create weights (alpha channel)
    # 0 at border, 1 at center
    w_y = np.ones(H)
    w_x = np.ones(W)
    
    # Taper edges
    taper_h = np.hanning(pad_h * 4) # Make a smooth transition
