"""
Solvers for Steepest Descent on Quotient Manifold (SDP)
"""

import numpy as np
from numpy.fft import fft2, ifft2

from .utils import (
    fft_convolve,
    project_kernel,
    threshold_kernel,
    center_kernel,
    tv_gradient,
    edge_taper,
    upsample_kernel,
    build_pyramid,
    kernel_shape_for_level,
    init_gaussian_kernel,
)


# ═══════════════════════════════════════════════════════════════════════
#  Gradient Computation
# ═══════════════════════════════════════════════════════════════════════

def compute_euclidean_gradients(
    y: np.ndarray, h: np.ndarray, x: np.ndarray
) -> tuple:
    """
    Euclidean gradients of data-fidelity term.
    """
    H_img, W_img = y.shape
    kh, kw = h.shape

    # Embed kernel centered at spatial origin for FFT convolution
    h_pad = np.zeros((H_img, W_img), dtype=np.float64)
    h_pad[:kh, :kw] = h
    h_pad = np.roll(h_pad, -(kh // 2), axis=0)
    h_pad = np.roll(h_pad, -(kw // 2), axis=1)

    F_h = fft2(h_pad)
    F_x = fft2(x)
    F_y = fft2(y)

    # Residual spectrum: F_y - F_h * F_x
    # Corresponds to -(h*x - y) in spatial domain
    F_r = F_y - F_h * F_x

    # ∇_x f = -IFT[ conj(F_h) * F_r ]
    grad_x = -np.real(ifft2(np.conj(F_h) * F_r))

    # ∇_h f = -IFT[ conj(F_x) * F_r ]
    # The result corresponds to the shifted kernel (at 0,0).
    # We must shift it back to the center (kh//2, kw//2) to match h's coordinates.
    grad_h_full = -np.real(ifft2(np.conj(F_x) * F_r))
    grad_h_full = np.roll(grad_h_full, kh // 2, axis=0)
    grad_h_full = np.roll(grad_h_full, kw // 2, axis=1)
    grad_h = grad_h_full[:kh, :kw]

    return grad_h, grad_x


# ═══════════════════════════════════════════════════════════════════════
#  Quotient-Manifold Operations
# ═══════════════════════════════════════════════════════════════════════

def horizontal_projection(
    grad_h: np.ndarray, grad_x: np.ndarray,
    h: np.ndarray, x: np.ndarray
) -> tuple:
    """
    Project onto horizontal space.
    Removes component parallel to the scaling orbit (h, -x).
    """
    inner_hg = np.sum(grad_h * h)
    inner_xg = np.sum(grad_x * x)
    norm_sq = np.sum(h ** 2) + np.sum(x ** 2)

    beta = (inner_hg - inner_xg) / (norm_sq + 1e-20) # Increased epsilon slightly

    # Horizontal component
    return grad_h - beta * h, grad_x + beta * x


# ═══════════════════════════════════════════════════════════════════════
#  Barzilai–Borwein Step Size
# ═══════════════════════════════════════════════════════════════════════

def barzilai_borwein_stepsize(
    s_h: np.ndarray, s_x: np.ndarray,
    z_h: np.ndarray, z_x: np.ndarray,
    iteration: int,
    alpha_min: float = 1e-12,
    alpha_max: float = 1e2, # Allow slightly larger steps initially
) -> float:
    """Adaptive step size."""
    ss = np.sum(s_h ** 2) + np.sum(s_x ** 2)
    sz = np.sum(s_h * z_h) + np.sum(s_x * z_x)
    zz = np.sum(z_h ** 2) + np.sum(z_x ** 2)

    if sz <= 1e-20:
        return alpha_max

    alpha_bb1 = ss / sz
    alpha_bb2 = sz / max(zz, 1e-20)

    # Alternate step sizes
    alpha = alpha_bb1 if iteration % 2 == 0 else alpha_bb2

    return float(np.clip(alpha, alpha_min, alpha_max))


# ═══════════════════════════════════════════════════════════════════════
#  Single-Scale SDP Solver
# ═══════════════════════════════════════════════════════════════════════

def sdp_single_scale(
    y: np.ndarray,
    h_init: np.ndarray,
    x_init: np.ndarray,
    lambda_tv: float = 0.002,
    max_iter: int = 300,
    tol: float = 1e-6,
    kernel_threshold: float = 0.0,
    verbose: bool = False,
) -> tuple:
    """Single-scale solver."""
    h = h_init.copy()
    x = x_init.copy()

    info = {'cost': [], 'kernel_diff': [], 'step_size': []}

    h_prev, x_prev = None, None
    g_h_prev, g_x_prev = None, None
    alpha = 1e-3

    for it in range(max_iter):
        h_old = h.copy()

        # 1. Gradients
        grad_h, grad_x = compute_euclidean_gradients(y, h, x)

        # 2. TV Regularization
        if lambda_tv > 0.0:
            grad_x = grad_x + lambda_tv * tv_gradient(x)

        # 3. Horizontal Projection
        g_h, g_x = horizontal_projection(grad_h, grad_x, h, x)

        # 4. BB Step Size
        if h_prev is not None:
            s_h = h - h_prev
            s_x = x - x_prev
            z_h = g_h - g_h_prev
            z_x = g_x - g_x_prev
            alpha = barzilai_borwein_stepsize(s_h, s_x, z_h, z_x, it)

        h_prev, x_prev = h.copy(), x.copy()
        g_h_prev, g_x_prev = g_h.copy(), g_x.copy()

        # 5. Update
        h = h - alpha * g_h
        x = x - alpha * g_x

        # 6. Constraints
        h = project_kernel(h)
        if kernel_threshold > 0.0:
            h = threshold_kernel(h, fraction=kernel_threshold)
        
        # Crucial: Clip image to valid range [0, 1]
        x = np.clip(x, 0.0, 1.0)

        # 7. Check Convergence
        diff = np.linalg.norm(h - h_old)
        cost = 0.5 * np.sum((y - fft_convolve(x, h)) ** 2)

        info['cost'].append(float(cost))
        info['kernel_diff'].append(float(diff))
        info['step_size'].append(float(alpha))

        if verbose and (it % 50 == 0 or it == max_iter - 1):
            print(f"    iter {it + 1:4d}/{max_iter}: "
                  f"cost={cost:.4e}  ΔH={diff:.2e}  α={alpha:.2e}")

        if diff < tol and it > 10:
            if verbose:
                print(f"    Converged at iteration {it + 1}.")
            break

    # Center kernel at the end of scale to remove shift ambiguity
    h = center_kernel(h)
    h = project_kernel(h)

    return x, h, info


# ═══════════════════════════════════════════════════════════════════════
#  Multi-Scale SDP Solver
# ═══════════════════════════════════════════════════════════════════════

def sdp_multiscale(
    y: np.ndarray,
    kernel_shape: tuple,
    lambda_tv: float = 0.002,
    num_scales: int = 4,
    iters_per_scale: int = 200,
    tol: float = 1e-6,
    kernel_threshold: float = 0.02,
    verbose: bool = False,
) -> tuple:
    """Coarse-to-fine solver."""
    pyramid = build_pyramid(y, num_scales=num_scales)
    num_levels = len(pyramid)

    if verbose:
        print(f"[SDP] Multi-scale blind deconvolution. Scales: {num_levels}")

    h_est = None
    history = {'scales': []}

    for level in range(num_levels):
        y_level = pyramid[level]
        ker_shape_l = kernel_shape_for_level(kernel_shape, level, num_levels)

        if verbose:
            print(f"\n  ── Scale {level} (img={y_level.shape}, ker={ker_shape_l}) ──")

        # Tapering (Now non-destructive)
        y_tapered = edge_taper(y_level, ker_shape_l)

        # Kernel init
        if h_est is None:
            h_level = init_gaussian_kernel(ker_shape_l)
        else:
            h_level = upsample_kernel(h_est, ker_shape_l)

        # Image init: Using observation is usually safer for blind deconv
        x_level = y_tapered.copy()

        # TV Weight Schedule (Higher at coarse scales)
        scale_progress = (level + 1) / num_levels
        lambda_tv_level = lambda_tv * (1.5 - 0.5 * scale_progress)

        x_level, h_level, info = sdp_single_scale(
            y_tapered, h_level, x_level,
            lambda_tv=lambda_tv_level,
            max_iter=iters_per_scale,
            tol=tol,
            kernel_threshold=kernel_threshold,
            verbose=verbose,
        )

        h_est = h_level
        history['scales'].append(info)

    # Final kernel resizing if needed
    if h_est.shape != kernel_shape:
        h_est = upsample_kernel(h_est, kernel_shape)
    h_est = center_kernel(h_est)
    h_est = project_kernel(h_est)

    # Final Non-Blind Refinement
    if verbose:
        print("\n  ── Final non-blind refinement ──")

    # Use original Y (tapered) for final restoration
    y_final_tapered = edge_taper(y, kernel_shape)
    x_est = refine_image_non_blind(
        y_final_tapered, h_est, y.copy(),
        lambda_tv=lambda_tv,
        max_iter=100,
        verbose=verbose,
    )

    return x_est, h_est, history


# ═══════════════════════════════════════════════════════════════════════
#  Non-Blind Image Refinement
# ═══════════════════════════════════════════════════════════════════════

def refine_image_non_blind(
    y: np.ndarray,
    h: np.ndarray,
    x_init: np.ndarray,
    lambda_tv: float = 0.002,
    max_iter: int = 100,
    verbose: bool = False,
) -> np.ndarray:
    """Non-blind deconvolution with known kernel."""
    H_img, W_img = y.shape
    kh, kw = h.shape

    h_pad = np.zeros((H_img, W_img), dtype=np.float64)
    h_pad[:kh, :kw] = h
    h_pad = np.roll(h_pad, -(kh // 2), axis=0)
    h_pad = np.roll(h_pad, -(kw // 2), axis=1)

    F_h = fft2(h_pad)
    F_hc = np.conj(F_h)
    F_y = fft2(y)

    # Lipschitz constant approximation
    # L = max|F_h|^2. 
    # Added small epsilon to avoid potential division issues in degenerate cases.
    L = np.max(np.abs(F_h) ** 2) + 1e-8
    step = 1.0 / L

    x = x_init.copy()

    for it in range(max_iter):
        F_x = fft2(x)
        F_r = F_y - F_h * F_x
        
        # Gradient: -IFT[ conj(F_h) * (F_y - F_h*F_x) ]
        grad = -np.real(ifft2(F_hc * F_r))

        if lambda_tv > 0.0:
            grad = grad + lambda_tv * tv_gradient(x)

        x = x - step * grad
        x = np.clip(x, 0.0, 1.0)

    if verbose:
        residual = y - np.real(ifft2(F_h * fft2(x)))
        cost = 0.5 * np.sum(residual ** 2)
        print(f"    Non-blind refinement done. Final fidelity={cost:.4e}")

    return x