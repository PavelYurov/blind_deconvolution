...
# ═══════════════════════════════════════════════════════════════════════
#  Non-Blind Image Refinement (Improved with FISTA)
# ═══════════════════════════════════════════════════════════════════════
...
def refine_image_non_blind(
    ...
    """
    Non-blind deconvolution using FISTA (Fast Iterative Shrinkage-Thresholding Algorithm).
    This converges significantly faster than standard Gradient Descent and handles 
    TV regularization much better, reducing ringing artifacts.
    """
    H_img, W_img = y.shape
    kh, kw = h.shape
    ...
    # Compute Lipschitz constant L = max|F_h|^2
    # This determines the maximum safe step size
    L_const = np.max(np.abs(F_h) ** 2) + 1e-8
    ...
    if verbose:
        print(f"    [Non-Blind] Starting FISTA refinement (iters={max_iter}, TV={lambda_tv})...")

    for it in range(max_iter):
        x_old = x.copy()

        # 1. Gradient descent step on Data Fidelity term based on y_fista
        # grad = H^T * (H * y_fista - y)
        F_y_fista = fft2(y_fista)
        F_r = F_h * F_y_fista - F_y  # Residual in freq domain (swapped sign compared to prev implementation)
        grad_data = np.real(ifft2(F_hc * F_r))

        # Gradient descent step
        z = y_fista - step_size * grad_data

        # 2. Proximal Step for TV Regularization
        # Instead of just adding gradient, we solve the TV proximal operator approx.
        # Simple approximation: x = z - step * lambda * grad_TV(z)
        # For FISTA, we apply this to the point 'z'
        if lambda_tv > 0.0:
            # We use the gradient of TV as a smooth approximation for the proximal step
            grad_tv = tv_gradient(z)
            x = z - step_size * lambda_tv * grad_tv
        else:
            x = z

        # Projection to [0, 1] box constraints
        x = np.clip(x, 0.0, 1.0)

        # 3. FISTA Momentum Update
        t_new = (1.0 + np.sqrt(1.0 + 4.0 * t_fista ** 2)) / 2.0
        y_fista = x + ((t_fista - 1.0) / t_new) * (x - x_old)
        t_fista = t_new

        # Optional: Print progress
        if verbose and (it % 20 == 0 or it == max_iter - 1):
            res_norm = np.linalg.norm(y - fft_convolve(x, h))
            print(f"    iter {it}: res={res_norm:.4e}")
