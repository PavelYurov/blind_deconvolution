...
class PMP_BD(DeconvolutionAlgorithm):
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # ── 3*. Sanitation cascade (experimental, shared module) ────────
        # When auto_mode='sanitation' the entire 3a–3g noise pipeline is
        # bypassed: the shared `_sanitation_cascade` module produces an
        # already-clean image and a residual-σ descriptor.  Stages 4–6
        # (blind kernel + non-blind) run normally on the cleaned image
        # using the post-cascade σ for λ tuning.
        ...
        if self.auto_mode == 'sanitation':
            ...
            sanitation_result = _sanitize(yg, verbose=self.verbose)
            ...
            if self.verbose:
                print(f"[PMP-BD] sanitation: branch="
        ...
        # Skip in 'sanitation' mode: σ is already post-cascade, and PMP's
        # legacy auto_params formulas (k·σ for λ_pmp/λ_grad) were tuned
        # for *pre*-denoised σ — they would over-tune at residual σ.
        if (self.auto_mode not in ('robust', 'sanitation')
        ...
        # Skip in 'sanitation' mode — input is already cleaned upstream.
