...
# ─────────────────────────────────────────────────────────────────────────────
# PSF library  (small kernels — k=21..25 typical for PMP)
# ─────────────────────────────────────────────────────────────────────────────
...
def _kernel_error(k_est: np.ndarray, k_true: np.ndarray) -> float:
    """L2 relative error after centroid alignment + size matching."""
    ...
    def _embed_centroid(k: np.ndarray, H: int, W: int) -> np.ndarray:
        """Embed k into HxW canvas with its centroid at the canvas center."""
        ...
        # destination centroid = (H-1)/2, (W-1)/2
        dy = (H - 1) / 2.0 - cy
        dx = (W - 1) / 2.0 - cx
        # nearest-int shift
        idy = int(round(dy))
...
def _evaluate(name: str,
    ...
    # OLD: vanilla PMP_BD with stock denoise pipeline OFF (purest "old"
    # baseline — no preprocessing, just blind+non-blind solver).
...
def main(seed: int = 0, kernel_size: int = 21, image_size: int = 256,
    ...
    # ── noise factory: row -> callable that takes (blurred) -> noisy ────
    cases = [
        ('AWGN σ=0.02',
