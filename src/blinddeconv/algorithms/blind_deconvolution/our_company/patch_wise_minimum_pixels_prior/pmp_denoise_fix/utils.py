...
def find_min_pixels(I: np.ndarray, patch_size: int, quantile: float = 0.0):
    ...
    Equivalent to MATLAB find_min_pixels.m when quantile=0.
    ...
    quantile   : float in [0, 1) — when 0, use absolute minimum
                 (original behaviour). When > 0, use the q-th quantile
                 value and pick the pixel closest to it. This is more
                 robust to noise outliers than the absolute minimum.
    ...
    for m in range(Mp):
        for n in range(Np):
            ...
            if quantile > 0 and flat.size > 1:
                # Robust quantile: pick the pixel closest to the q-th
                # percentile instead of the absolute minimum.
...
def threshold_pxpy_v1(latent: np.ndarray, psf_size,
    ...
    if denoise_eps is not None and denoise_eps > 0:
        ...
        if ensemble_denoise:
            # 3 guided filters at varied (radius, eps) — all edge-preserving
