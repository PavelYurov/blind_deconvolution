...
# Import the orchestrator + the test image source
...
def _add_colored_gaussian(clean: np.ndarray, sigma: float, seed: int,
    ...
    """Spatially correlated Gaussian noise via Gaussian-blurred white noise."""
    from scipy.ndimage import gaussian_filter
    rng = np.random.default_rng(seed)
    white = rng.normal(0, 1.0, clean.shape)
    coloured = gaussian_filter(white, sigma=smooth_radius)
    # Re-scale to target std σ
...
def _add_poisson(clean: np.ndarray, photons: float, seed: int) -> np.ndarray:
    """Pure Poisson noise.  ``photons`` = expected photon count at intensity=1."""
    rng = np.random.default_rng(seed)
...
def _add_periodic(clean: np.ndarray, sigma: float, seed: int,
    ...
    """White Gaussian + a sinusoidal interference pattern."""
    rng = np.random.default_rng(seed)
    H, W = clean.shape
...
def main():
    ...
    for name, noisy, expect in cases:
        ...
        try:
            ...
            psnr_ok = psnr_out > psnr_in - 0.5  # allow tiny degradation
