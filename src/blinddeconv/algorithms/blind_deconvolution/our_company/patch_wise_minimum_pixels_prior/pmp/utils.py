...
круговое граничное сглаживание для минимизации краевых эффектов, двустороннюю 
фильтрацию, а также специализированный оператор поиска минимальных пикселей 
в неперекрывающихся паттернах.
"""
...
def wrap_boundary_liu(img: np.ndarray, img_size: tuple) -> np.ndarray:
    ...
    Формирует гладкие переходы между противоположными краями изображения
    путем интерполяции и решения уравнения Пуассона, что предотвращает
    появление высокочастотных артефактов при спектральной обработке.
    """
    if img.ndim == 2:
        img = img[:, :, np.newaxis]
...
def find_min_pixels(I: np.ndarray, patch_size: int):
    """
    Поиск минимальных пикселей в неперекрывающихся паттернах (основа априорного знания PMP).

    Разбивает изображение на неперекрывающиеся блоки заданного размера и
    идентифицирует пиксель с минимальной интенсивностью в каждом из них.
    Сформированная разреженная матрица используется как целевой вектор 
    для L0-регуляризации в задаче восстановления скрытого изображения.
    ...
    ---------------------
    J : ndarray
        Разреженное изображение размерности (M, N), где ненулевыми являются только
        элементы, соответствующие найденным локальным минимумам (содержат их значения).
    Mask : ndarray
...
def threshold_pxpy_v1(latent: np.ndarray, psf_size,
    ...
    Вычисляет горизонтальные и вертикальные пространственные градиенты.
    Если порог отсечения не задан, он вычисляется автоматически на основе
    статистического анализа распределения амплитуд градиентов в четырех
    угловых секторах.
    """
