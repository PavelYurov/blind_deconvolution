...
class DCP_BD(DeconvolutionAlgorithm):
    ...
    def __init__(
        ...
        # Used by _orchestrate_robust() to restore on the 'clean' branch and
        # to compute the (1-w)*default + w*noisy blend on the 'heavy' branch.
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # ── 3b. Noise estimation ────────────────────────────────────────
        # Auto-promote to PCA when robust orchestrator is enabled and user
        # forgot to enable noise estimation explicitly.
        ...
        # Decides preprocess / act_preprocess / pre_nonblind / blind_hooks /
        # NB-weights based on noise_info.  Runs AFTER user's auto_params so
        # the NB-blend can detect the user-active branch and skip itself.
    ...
    def _orchestrate_robust(self, noise_info):
        ...
        Manages 6 groups:
            1. preprocess / preprocess_params
            2. pre_nonblind / pre_nonblind_params
            3. act_preprocess / act_params
            4. final_deconv / nb_params
            5. blind_hooks (full replacement on heavy branch)
            6. ringing weights (lambda_tv, lambda_l0, weight_ring) — blended
               UNLESS user's `auto_params` is active (it owns lambdas).

        Untouched: kernel_size, lambda_dark, lambda_grad, xk_iter,
        gamma_correct, k_thresh (paper core),
        impulse_preprocess, screenot_preprocess, noise_preprocess,
        histogram_eq (orthogonal user knobs).

        Returns
        -------
        dict — diagnostic record stored in hyperparams['orchestrator_info'].
        """
        ...
        # ── Clean branch: restore user-provided values, paper-pure run ──
        if not heavy:
            info['branch'] = 'clean'
        ...
        # Final NB method and nb_params: keep user choice.
        self.final_deconv = snap['final_deconv']
        self.nb_params    = snap['nb_params']

        if poisson_like or prefer_act:
            # ACT route: curvelet pre + ACT pre-NB; light bilateral in loop
            info['route'] = 'act'
            ...
            # Gaussian route: split light / strong by blend weight
        ...
        # NB-blend for ringing_removal weights — only if user's auto_params
        # is NOT active (otherwise it owns lambda_tv/lambda_l0 already).
