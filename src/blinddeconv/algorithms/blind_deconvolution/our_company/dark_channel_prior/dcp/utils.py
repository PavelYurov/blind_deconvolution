...
def _build_opt_fft_lut(lut_size: int = 4096) -> np.ndarray:
    """Формирование справочной таблицы оптимальных размеров БПФ."""
...
def _solve_min_laplacian(boundary_image: np.ndarray) -> np.ndarray:
    ...
    дискретное синус-преобразование (DST).
    """
...
def dark_channel(I: np.ndarray, patch_size: int):
    ...
    в заданной локальной окрестности (среди всех цветовых каналов).
    Возвращает само значение и его линейный индекс в столбцовом порядке 
    внутри блока.

    Параметры
    ---------
    I : ndarray
        Изображение размерности (M, N) или (M, N, C).
    patch_size : int
        Размер окна поиска (нечетное число).

    Возвращает
    ----------
    J : ndarray
        Изображение темного канала размерности (M, N).
    J_index : ndarray
        Линейный индекс минимального элемента внутри каждого окна.
    """
    if I.ndim == 2:
        I = I[:, :, np.newaxis]
...
def dark_channel_fast(I: np.ndarray, patch_size: int):
    """
    Векторизованная версия функции вычисления темного канала.
    Индексы формируются в столбцовом порядке.
    """
    if I.ndim == 2:
        I = I[:, :, np.newaxis]
...
def assign_dark_channel_to_pixel(S: np.ndarray,
    ...
    """
    Присвоение уточненных значений темного канала обратно пикселям 
    изображения на основе сохраненных индексов.

    Параметры
    ---------
    S : ndarray
        Текущая оценка изображения размерности (M, N) или (M, N, C).
    dark_channel_refine : ndarray
        Уточненный темный канал размерности (M, N).
    dark_channel_index : ndarray
        Индексы минимальных элементов внутри блоков (столбцовый порядок).
    patch_size : int
        Размер использованного окна.

    Возвращает
    ----------
    outImg : ndarray
        Обновленное изображение (формат совпадает с S).
    """
    if S.ndim == 2:
        S_3d = S[:, :, np.newaxis]
    else:
...
def conjgrad(x: np.ndarray, b: np.ndarray, max_it: int, tol: float,
    ...
    Умножение на матрицу A задается пользовательской функцией.
    """
    x = x.copy()
...
def adjust_psf_center(psf: np.ndarray) -> np.ndarray:
    """
    Центрирование PSF путем вычисления ее центра масс и его сдвига 
    к геометрическому центру массива.
    Используется билинейная интерполяция для нецелочисленных смещений.
    """
    rows, cols = psf.shape
...
def _histc(data: np.ndarray, edges: np.ndarray) -> np.ndarray:
    """
    Вспомогательная функция для построения гистограммы с включением 
    граничных значений в последний интервал.
    """
...
def threshold_pxpy_v1(latent: np.ndarray, psf_size,
    ...
    в процессе оценки функции рассеяния точки.

    Слабые градиенты, величина которых меньше вычисленного порога, обнуляются. 
    Если порог не задан, он оценивается автоматически на основе кумулятивной 
    гистограммы градиентов по четырем основным направлениям.

    Параметры
    ---------
    latent : ndarray
        Текущая оценка изображения размерности (M, N).
    ...
    ----------
    px, py : ndarray
        Градиентные изображения с обнуленными слабыми перепадами.
    threshold : float
...
def _fspecial_gaussian(size: int, sigma: float) -> np.ndarray:
    """Формирование двумерного ядра Гаусса."""
    radius = (size - 1) / 2.0
...
def bilateral_filter(img: np.ndarray, sigma_s: float,
    ...
    """
    Билатеральный фильтр для подавления шума с сохранением краев объектов.
    """
    if img.ndim == 2:
        img = img[:, :, np.newaxis]
...
def graythresh(img: np.ndarray) -> float:
    ...
    ----------
    threshold : float
        Пороговое значение в диапазоне [0, 1].
    """
...
def tikhonov_filter(img: np.ndarray, kernel: np.ndarray,
    ...
    первого порядка). Решение выполняется аналитически в частотной области.
    """
    H, W = img.shape[:2]
    otf = psf2otf(kernel, (H, W))
...
def edgetaper(img: np.ndarray, kernel: np.ndarray,
    ...
    предотвращения артефактов звона при БПФ-деконволюции.
    """
    H, W = img.shape[:2]
    kh, kw = kernel.shape
