...
def blind_deconv_main_sr_enhanced(blur_B, k, lambda_dark, lambda_grad,
    ...
    gradients.  This injects high-frequency detail from the SR
    reference (which was built from a partially-deblurred image)
    into the gradient maps, giving the kernel estimator sharper
    and more physically consistent edges to work with.
    ...
    for _iter in range(xk_iter):
        ...
        # ── Step 2 (SR ENHANCEMENT): Blend with SR reference ─────────
        # The SR reference was built from a Wiener-deblurred image
        # (not the raw blurred input!), so it contains genuine
        # scene structure.  Blending it into S before gradient
        # extraction reinforces real edges without adding false ones.
...
def _build_sr_reference(y, ks_warmup, k1list, k2list, ret, retv, warmup_level,
    ...
    2. Wiener-deconvolve the original blurred image → partially clean.
    3. Run SelfExSR on the partially clean image → enhanced reference.
    4. Convert to grayscale and resize to original dimensions.
    ...
    wiener_snr     : Wiener filter noise-to-signal ratio
    ...
    # This gives us an image that is roughly deblurred — still has some
    # ringing/noise, but the SCENE STRUCTURE is restored.  This is
    # what SelfExSR needs: a degraded-but-structurally-correct input
    # where self-similar patches reflect real scene content.
    S_deconv = wiener_filter(y, k_full, noise_snr=wiener_snr)
    S_deconv = np.clip(S_deconv, 0.0, 1.0)

    # ── Step 3: Run SelfExSR on the partially deblurred image ────────
    ...
    y_sr_hr = sr_demo(S_deconv, SRF, opt=sr_opt)

    # ── Step 4: Convert to grayscale, resize to original ─────────────
    if y_sr_hr.ndim == 3:
