...
# Compiled .pyd files are imported as ``esm_cython._build_pyd.<module>``
# (the same convention used by the DCP cython port).  No sibling
# ``esm_cython_pyd`` package — we install only into ``_build_pyd/``.
INSTALL_DIR = PKG_DIR.parent / "esm_cython_pyd"
