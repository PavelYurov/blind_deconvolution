...
class AEER_BD(DeconvolutionAlgorithm):
    def __init__(
        ...
        # Метод финального восстановления: 'wiener', 'tikhonov', 'admm'
        final_deconv: str = 'tikhonov', 
        nb_iter: int = 100,
        debug: bool = True,
        debug_dir: str = "debug_aeer"
    ):
        super().__init__(name='AEE-BD')
    ...
    def process(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 2. Loop Preparation (Using EDGETAPER)
        ...
        for n in range(self.max_iter):
            ...
            if self.debug and (n % 5 == 0):
                self._save_debug_image(u, "u_loop", n+1)
        ...
        if self.final_deconv == 'wiener':
            ...
            # alpha управляет гладкостью. 0.01 - мягко, 0.05 - сильнее.
            ...
            for i in range(self.nb_iter):
                u_prev_p = u_p.copy()
                u_p = solve_u_subproblem(k_final_pad, f_pad, p_p, l1_p, self.r1, self.lambda_val, OTF_dx_p, OTF_dy_p)
                u_p = np.clip(u_p, 0, 1)
