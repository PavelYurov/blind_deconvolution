"""
api.py — self-contained sanitation cascade.

Pipeline (each step is gated by an empirical assumption — no blind heuristics):

    [0] JPEG-floor pre-gate   : Chen σ < noise_floor → no-op.
    [1] impulse pre-pass      : adaptive median on flagged pixels only.
    [2] periodic notch        : notch peaks gated by lag-1 broadband test.
    [3] descriptors on residual: Pyatykh PCA + Chen σ + PSD analysis.
    [4] branch:
            poisson family + a_norm > _MIN_A_FOR_VST  → VST + BM3D + inverse
            truly correlated AND σ ≥ act_colored_min_sigma → ACT-colored
            white with σ > noise_floor                → BM3D-white
            otherwise                                  → no-op

Public API
----------
    sanitize(image, *, profile='auto', ...) -> SanitationResult
    SanitationResult                          (dataclass)

All detector / denoiser modules live in this package — no external
dependencies on `pmp_denoise_fix`.
"""
...
# Co-located detector / denoiser modules.
...
# ─────────────────────────────────────────────────────────────────────────────
# Default thresholds (see module docstring of `_sanitation_cascade` for
# calibration notes).
# ─────────────────────────────────────────────────────────────────────────────

# JPEG-grade noise floor (Chen σ in [0,1] units).  Below this, the image
# is essentially clean (or only carries quantisation/JPEG noise that no
# Gaussian denoiser should attempt to remove) — the cascade short-circuits.
# Same value as LIP `_orchestrate_robust` `sigma_clean`.
_DEFAULT_NOISE_FLOOR = 0.005

# Above this Chen σ we are confident there is real noise; correlation
# tests on the residual become trustworthy.
_BORDERLINE_SIGMA = 0.010

# In the borderline regime (noise_floor ≤ σ < borderline) we require a
# higher lag-1 to engage the colored branch — natural image leakage gives
# lag-1 ≈ 0.4–0.5 even on clean inputs.
_BORDERLINE_LAG1_GATE = 0.65

# Heavy-σ threshold above which ACT-colored is preferred to BM3D-white
# for confirmed correlated noise.  Below this level BM3D-white with
# scalar σ produces equal or better detail preservation.
_ACT_COLORED_MIN_SIGMA = 0.020

# Cascade thresholds (carried over verbatim from the validated PMP
# orchestrator — calibration described in the original module).
_MIN_A_FOR_VST = 1e-6                 # min Poisson gain to engage VST
_MIN_IMPULSE_DENSITY = 5e-3           # 0.5 % impulse-pixel fraction
_DEFAULT_PEAK_THRESHOLD = 2000.0      # periodic-peak ratio
_PERIODIC_LAG1_GATE = 0.5             # high lag-1 → broadband colored, skip notch
_TRUE_CORRELATION_LAG1 = 0.5          # baseline (raised to 0.65 in borderline σ)
_TRUE_CORRELATION_CV = 0.3            # radial PSD coefficient of variation
...
class SanitationResult:
    ...
    Attributes
    ----------
    image_clean : ndarray (H, W) float64 in [0, 1]
        Grayscale denoised image, ready to feed into a blind-deconv stage.
    noise_info : dict
        Schema-compatible with what algorithms expect from their own
        `_estimate_noise`: at minimum contains ``sigma_norm`` (post-cascade
        Chen σ in [0,1] units) and ``noise_type``.
    actions : list[str]
        Human-readable trace of cascade decisions.
    residual_sigma : float
        Chen σ on the cleaned output, in [0, 1] units.  Use this — NOT
        the pre-cascade σ — when configuring λ/τ/α downstream.
    residual_type : {'awgn', 'mild_colored', 'clean'}
        Best-effort label for the residual character after the cascade.
    branch : str
        Branch taken ('vst' | 'act_colored' | 'bm3d_white' | 'noop').
    raw : dict
        Full diagnostic info (impulse / psd / pca descriptors).  Schema
        is private — do not rely on it.
    """
...
def _psd_to_act_fft_format(psd_centered: np.ndarray) -> np.ndarray:
    """Convert centred PSD (σ² for white) to ACT FFT order (σ²·H·W)."""
...
def _is_truly_correlated(psd_info: dict) -> bool:
    """Two-stage colored-noise test — see PMP noise_orchestrator docs.

    Both criteria must hold:
        1. ``max(|lag1_h|, |lag1_v|) ≥ _TRUE_CORRELATION_LAG1``
        2. ``CV(radial_psd_midband) ≥ _TRUE_CORRELATION_CV``
    """
...
def _denoise_poisson_vst(image: np.ndarray,
    ...
    """VST + BM3D + asymptotic inverse (Mäkitalo-Foi 2013)."""
...
def sanitize(image: np.ndarray,
    ...
    """Run the full sanitation cascade with conservative safeguards.

    Parameters
    ----------
    image : ndarray
        Noisy input — gray or RGB, any scale (auto-normalised internally).
    profile : {'auto'}
        Reserved for future presets ('aggressive', 'preserve_edges', ...).
    noise_floor : float, default 0.005 (=1.25/255)
        JPEG-grade Chen σ floor in [0,1] units.  Below this, the cascade
        is a NO-OP.  Prevents false positives where natural image content
        (1/f power, smooth gradients) fools the correlation test.
    borderline_sigma : float, default 0.010
        Below this, colored-branch detection is gated by the stricter
        `borderline_lag1_gate` threshold.
    borderline_lag1_gate : float, default 0.65
        Lag-1 threshold for engaging ACT-colored in the borderline regime.
        Real correlated noise gives lag-1 ≥ 0.7–0.9; natural image leakage
        caps near 0.5.
    act_colored_min_sigma : float, default 0.020
        Minimum Chen σ for using ACT-colored.  Below this, even if
        correlation is detected, BM3D-white is used — it preserves detail
        better at moderate noise levels.
    ...
    # ── Stage 4: branch routing with conservative gates ─────────────
    ...
    # ── Stage 5: residual descriptor on cleaned output ──────────────
