...
def _interpolate_library(blurred, kernel, double alpha, double alpha_n, lam_map, lam_library):
    cdef double C = lam_library[0]
    ...
    for m in range(M):
        for n in range(N):
            i = idx_view[m, n]
            lam_val = lam_view[m, n]
            if lam_val < 1e-12: lam_val = 1e-12

            lib_i = lib_view[i]
            if lib_i < 1e-12: lib_i = 1e-12
            lib_next = lib_view[i + 1]

            denom = log(lib_next / lib_i)
            ...
            val_i = img_i[m, n]
            val_next = img_next[m, n]

            out_view[m, n] = w * val_i + (1.0 - w) * val_next
