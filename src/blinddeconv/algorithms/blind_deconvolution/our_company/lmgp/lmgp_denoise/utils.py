...
def threshold_pxpy_v1(latent: np.ndarray, psf_size,
    ...
    if denoise_eps is not None and denoise_eps > 0:
        if denoise_type == 'bm3d':
            ...
            denoised = nlm_filter(latent, h=nlm_h)
...
def nlm_filter(img: np.ndarray, h: float = 0.01,
    ...
    return denoise_nl_means(
        img, h=h * sigma_est / 0.01, sigma=sigma_est,
    ...
    if softmax_tau is not None and softmax_tau > 0:
        ...
        for n_0 in range(N):            # col-major outer
            for m_0 in range(M):        # col-major inner
                ...
                lr = np.arange(h1)
                lc = np.arange(h2)
                gr, gc = np.meshgrid(r_s + lr, c_s + lc, indexing='ij')
                global_idx = gr.flatten(order='F') + gc.flatten(order='F') * M

                row_idx = m_0 + n_0 * M
                row_list.append(np.full(len(global_idx), row_idx,
                                        dtype=np.int64))
                col_list.append(global_idx.astype(np.int64))
                val_list.append(w)
