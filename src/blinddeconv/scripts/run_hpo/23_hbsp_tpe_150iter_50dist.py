"""
Оптимизация гиперпараметров алгоритма BID-HBSP (Blind Image Deconvolution,
Hyperbolic-Secant Prior)  с использованием TPE.
...
from blinddeconv.algorithms.blind_deconvolution.our_company.bayesian.bid_hbsp_denoise.bid_hbsp import BID_HBSP
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "23_hbsp_tpe_150iter_50dist"
...
# --- Фиксированные параметры алгоритма BID_HBSP ---
# Настройки, не участвующие в байесовской оптимизации.
# В режиме auto_mode='robust' оркестратор автоматически активирует PCA-оценку шума 
_BID_HBSP_FIXED_PARAMS: Dict[str, Any] = {
    "impulse_preprocess": "auto",      # Автоматическая детекция и удаление импульсного шума
    "auto_mode":          "robust",    # Активация робастного шумоподавляющего оркестратора
    "final_deconv":       "ringing",   # Использование алгоритма подавления краевых артефактов (ringing removal)
}


# --- Пространство поиска гиперпараметров (HP Space) ---
# Обоснование диапазонов базируется на вариационном байесовском выводе (VB-EM) 
# для априорного распределения гиперболического секанса (HS).
BID_HBSP_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Параметры пространственной поддержки ──
    "kernel_size":      {"type": "int",   "values": [15, 61]},       # Размер ядра PSF (преобразуется в нечётный кортеж)

    # ── Параметры байесовской модели (HS-prior и модель шума) ──
    "hs_scale":         {"type": "float", "values": [0.004, 0.02]},  # Масштабный параметр b распределения HS (α = 1/b). Контролирует степень разреженности градиентов
    "noise_sigma":      {"type": "float", "values": [0.001, 0.02]},  # Начальное СКО гауссова шума. Инициализирует точность шума β_0 = 1/σ^2

    # ── Управление итерационными решателями ──
    "max_iter":         {"type": "int",   "values": [20, 70]},       # Максимальное количество внешних итераций VB-EM на каждом масштабе
    "cg_iter":          {"type": "int",   "values": [30, 120]},      # Максимальное количество итераций метода сопряженных градиентов (CG)
    "cg_tol":           {"type": "float", "values": [1e-7, 5e-5]},   # Порог сходимости (толерантность) для CG-решателя
    "irw_iter":         {"type": "int",   "values": [3, 10]},        # Количество итераций IRLS для финального неслепого восстановления

    # ── Регуляризация оценки ядра (Tikhonov regularization + Annealing) ──
    "lambda_h_init":    {"type": "float", "values": [20.0, 300.0]},  # Начальный вес L2-регуляризации ядра в задаче квадратичного программирования (QP)
    "lambda_h_decay":   {"type": "float", "values": [0.85, 0.98]},   # Коэффициент экспоненциального затухания регуляризации ядра
    "beta_n_factor":    {"type": "float", "values": [1.2, 4.0]},     # Коэффициент масштабирования точности шума для пространства фильтров (β_n = β / factor)
    ...
    "weight_ring":      {"type": "float", "values": [0.2, 1.2]},     # Весовой коэффициент вычитания артефактов звона (ringing subtraction)
}
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    ...
    # Сбор параметров алгоритма BID_HBSP из конфигурации
    lr_params: Dict[str, Any] = {}
    for name in dict_config:
        lr_params[name] = config[name]

    # int-параметры
    _INT_KEYS = {"kernel_size", "max_iter", "cg_iter", "irw_iter"}
    for ik in _INT_KEYS:
        if ik in lr_params:
            lr_params[ik] = int(round(lr_params[ik]))

    # kernel_size -> kernel_shape (square, odd)
    if "kernel_size" in lr_params:
        ks = lr_params["kernel_size"]
        if ks % 2 == 0:
            ks += 1
        ks = max(ks, 3)
        lr_params["kernel_shape"] = (ks, ks)
        lr_params.pop("kernel_size", None)

    # Non-blind ringing params are passed via nb_params.
    nb_params: Dict[str, Any] = {}
    for k in ("lambda_tv", "lambda_l0", "weight_ring"):
        if k in lr_params:
            nb_params[k] = float(lr_params.pop(k))
    if nb_params:
        lr_params["nb_params"] = nb_params

    # Применение фиксированных параметров алгоритма
    lr_params.update(_BID_HBSP_FIXED_PARAMS)

    print(f"\n{'='*72}")
    print(f"  [eval {eval_id}] BID_HBSP params: {lr_params}")
    print(f"{'='*72}")
