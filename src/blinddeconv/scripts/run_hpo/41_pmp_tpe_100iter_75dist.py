...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "41_pmp_tpe_100iter_75dist"
...
# Фиксированные параметры алгоритма PMP_BD (не оптимизируемые).
# Обоснование:
# pmp_quantile     : Установлено в 0.0, что означает поиск строгого локального минимума (классический PMP).
# ensemble_denoise / blind_denoise: PMP очень чувствителен к шуму, поэтому внутри слепого цикла используется ансамбль из 3 Guided фильтров.
# denoise_eps / denoise_radius: Фиксированные параметры для встроенного Guided-фильтра.
# act_preprocess   : Использование Curvelet-денойзера перед началом работы алгоритма.

_PMP_FIXED_PARAMS: Dict[str, Any] = {
    "gamma_correct":      1.0,              # Выключение гамма-коррекции перед оценкой ядра
    "pmp_quantile":       0.0,              # Использовать строгий минимум в патче (0-й квантиль)
    "ensemble_denoise":   True,             # Использовать ансамбль из 3х Guided фильтров для вычисления градиентов
    "impulse_preprocess": "auto",           # Очистка от salt & pepper (критично для PMP, иначе минимум будет равен 0)
    "noise_estimation":   "pca",            # Оценка уровня шума методом PCA
    "act_preprocess":     "auto",           # Применение Adaptive Curvelet Thresholding перед слепой деконволюцией
    "blind_denoise":      "guided",         # Использование Guided-фильтра внутри каждой слепой итерации
    "auto_mode":          "robust",         # Адаптация весов в зависимости от шума
    "final_deconv":       "ringing_removal",# Использовать метод подавления звона (TV + L0 + bilateral)
    "denoise_eps":        0.006,            # Регуляризация (ε) для Guided-фильтра
    "denoise_radius":     2,                # Радиус окна для Guided-фильтра
    "nb_params": {
        "lambda_tv":   0.005,               # Базовый вес TV для non-blind шага
        "lambda_l0":   0.002,               # Базовый вес L0 для non-blind шага
        "weight_ring": 1.0,                 # Сила вычитания артефактов звона
    },
    "auto_mode_params": {
        "force_heavy_sigma": 0.02,          # Принудительное включение агрессивного режима при шуме > 2%
    },
}

# --- Пространство поиска гиперпараметров (HP Space) ---
# Обоснование диапазонов:
# lambda_pmp        : Вес PMP-априорного распределения (интенсивности). Отвечает за принуждение локальных патчей иметь темные/минимальные пиксели (устраняет размытие).
# lambda_grad       : Вес L0-градиентного априорного распределения. Удаляет мелкий шум и сохраняет сильные контуры (кусочно-постоянные участки).
# patch_r           : Размер (радиус) окна поиска минимального пикселя. Слишком маленькое окно не захватит край, слишком большое исказит структуру.
# xk_iter           : Количество альтернирующих шагов (Изображение ↔ Ядро) на один масштаб пирамиды.
# k_thresh          : Обратный порог отсечения шума в ядре (значения ниже max(k)/k_thresh зануляются).
# grad_smooth_sigma : Сила предварительного Гауссова сглаживания градиентов размытого изображения перед оценкой ядра (повышает робастность к шуму).

PMP_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Размер ядра ──
    "kernel_size":       {"type": "int",   "values": [21, 51]},

    # ── Основные веса регуляризации ──
    "lambda_pmp":        {"type": "float", "values": [4e-3, 0.5]},             # Вес априорного распределения патчевых минимумов (PMP)
    "lambda_grad":       {"type": "float", "values": [1e-4, 2e-2]},            # Вес L0-градиентной регуляризации
    
    # ── Пространственные и итерационные параметры ──
    "patch_r":           {"type": "int",   "values": [3, 9]},                  # Радиус патча для PMP
    "xk_iter":           {"type": "int",   "values": [5, 20]},                 # Чередования ядро-изображение

    # ── Постобработка и стабильность оценки ядра ──
    "k_thresh":          {"type": "float", "values": [10.0, 50.0]},            # Делитель порога отсечения ядра (max/k_thresh)
    "grad_smooth_sigma": {"type": "float", "values": [0.0, 2.5]},              # Гауссово сглаживание градиентов (0 = выкл)
}
...
def _plot_pareto_3d_with_points(
    ...
    for view_idx, (elev, azim) in enumerate(_VIEW_ANGLES, 1):
        ...
        print(f"  [Pareto] сохранено: {path.name}")
    plt.close(fig)
...
def _plot_pareto_surface_only(
    ...
    for view_idx, (elev, azim) in enumerate(_VIEW_ANGLES, 1):
        ...
        plt.savefig(str(path), dpi=150, bbox_inches="tight", pad_inches=0.4)
        print(f"  [Pareto] сохранено (только поверхность): {path.name}")
    plt.close(fig)
...
def _plot_pareto_3d_with_points_nd(
    ...
    for view_idx, (elev, azim) in enumerate(_VIEW_ANGLES, 1):
        ...
        plt.savefig(str(path), dpi=150, bbox_inches="tight", pad_inches=0.4)
        print(f"  [Pareto ND] сохранено: {path.name}")
    plt.close(fig)
...
def _plot_pareto_surface_only_nd(
    ...
    for view_idx, (elev, azim) in enumerate(_VIEW_ANGLES, 1):
        ...
        plt.savefig(str(path), dpi=150, bbox_inches="tight", pad_inches=0.4)
        print(f"  [Pareto ND] сохранено (только поверхность): {path.name}")
    plt.close(fig)
...
def update_pareto_plots(iteration: int) -> None:
    ...
    if _HAS_PLOTLY:
        ...
        for metric, pts, mask in [("PSNR", psnr_pts, psnr_mask),
            ...
            print(f"  [Pareto] сохранён интерактивный: {html_path.name}")
    else:
        print("  [Pareto] plotly не установлен — интерактивные HTML пропущены")
    ...
    if _HAS_PLOTLY:
        ...
        for metric, pts, mask in [("PSNR", nd_psnr_pts, nd_psnr_mask),
            ...
            fig.write_html(str(html_path))
            print(f"  [Pareto ND] сохранён интерактивный: {html_path.name}")
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    ...
    # int-параметры
    _INT_KEYS = {"kernel_size", "xk_iter", "patch_r"}
    for ik in _INT_KEYS:
        if ik in lr_params:
