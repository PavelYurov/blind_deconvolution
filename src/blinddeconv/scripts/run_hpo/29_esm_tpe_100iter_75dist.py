"""
Оптимизация гиперпараметров алгоритма ESM-BD (Enhanced Sparse Model) с использованием TPE.
...
from blinddeconv.algorithms.blind_deconvolution.our_company.esm_denoise.esm import ESM_BD
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "29_esm_tpe_100iter_75dist"
...
# --- Фиксированные параметры алгоритма ESM_BD ---
# Базовые настройки алгоритма, не участвующие в процессе байесовской оптимизации.
# В режиме auto_mode='robust' оркестратор автоматически оценивает шум (PCA).
_ESM_FIXED_PARAMS: Dict[str, Any] = {
    "gamma_correct":      1.0,                 # Отключение гамма-коррекции перед оценкой ядра (identity mapping)
    "final_deconv":       "ringing_removal",   # Использование алгоритма подавления артефактов звона (базовый метод ESM)
    "impulse_preprocess": "auto",              # Адаптивный медианный фильтр для удаления импульсного шума до слепого цикла
    "auto_mode":          "robust",            # Активация робастного шумоподавляющего оркестратора
    "verbose":            False,               # Отключение детализированного вывода в консоль
}

# --- Пространство поиска гиперпараметров (HP Space) ---
# Обоснование диапазонов базируется на математической модели Enhanced Sparse Prior (ECCV 2020),
# которая использует аппроксимацию L0-L1 нормы для градиентов и невязки данных.
ESM_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Параметры пространственной поддержки ──
    "kernel_size":      {"type": "int",   "values": [15, 61]},       # Размер ядра PSF (преобразуется в нечётный кортеж в целевой функции)

    # ── Параметры модели Enhanced Sparse Prior (ℓ0–ℓ1) ──
    "lambda_data":      {"type": "float", "values": [1e-4, 5e-2]},   # Вес ℓ0–ℓ1 регуляризатора невязки градиентов данных: (k * ∇I − ∇B)
    "lambda_grad":      {"type": "float", "values": [1e-4, 5e-2]},   # Вес ℓ0–ℓ1 регуляризатора пространственных градиентов латентного изображения: ∇I
    "theta":            {"type": "float", "values": [0.1, 5.0]},     # Параметр формы θ для функции аппроксимации усиленной разреженности (enhanced sparsity)

    # ── Управление итерационным процессом ──
    "xk_iter":          {"type": "int",   "values": [2, 10]},        # Количество внутренних чередований обновления изображения (I) и ядра (k) на одном масштабе
    ...
    "lambda_tv":        {"type": "float", "values": [1e-4, 5e-2]},   # Вес изотропной Total Variation (TV) регуляризации в неслепом шаге
    "lambda_l0":        {"type": "float", "values": [1e-4, 1e-2]},   # Вес разреживающего L0-градиентного регуляризатора в неслепом шаге
    "weight_ring":      {"type": "float", "values": [0.0, 2.0]},     # Весовой коэффициент вычитания артефактов звона (0.0 = классическая TV-деконволюция)
}
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    ...
    # Сбор параметров алгоритма ESM_BD из конфигурации
    lr_params: Dict[str, Any] = {}
    for name in dict_config:
        lr_params[name] = config[name]

    # int-параметры
    _INT_KEYS = {"kernel_size", "xk_iter"}
    for ik in _INT_KEYS:
        if ik in lr_params:
    ...
    lr_params.update(_ESM_FIXED_PARAMS)

    print(f"\n{'='*72}")
    print(f"  [eval {eval_id}] ESM_BD params: {lr_params}")
    print(f"{'='*72}")
