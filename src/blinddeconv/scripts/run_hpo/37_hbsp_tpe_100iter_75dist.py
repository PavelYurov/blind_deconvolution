"""
Оптимизация гиперпараметров алгоритма BID-HBSP (Bayesian Image Deconvolution, Half-quadratic Bayesian Sparse Prior)
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "37_hbsp_tpe_100iter_75dist"
...
# Фиксированные параметры алгоритма BID_HBSP (не оптимизируемые).
# Обоснование:
# auto_mode="robust" : Включает оркестратор, который переключает пайплайн между "чистым" режимом (классический HBSP) и "шумным" (добавление денойзеров).
# final_deconv       : По умолчанию HBSP использует FIRLS, но "auto" позволит оркестратору переключиться на ringing_removal при сильном шуме.
# nb_params          : Включает в себя параметры как для FIRLS (lam, alpha), так и для Ringing Removal (lambda_tv, lambda_l0).

_BID_HBSP_FIXED_PARAMS: Dict[str, Any] = {
    "impulse_preprocess": "auto",           # Фильтрация битых пикселей до оценки ядра
    "auto_mode":          "robust",         # Адаптивный выбор денойзеров в зависимости от уровня шума
    "final_deconv":       "auto",           # Авто-выбор между FIRLS и Ringing Removal
    "nb_params": {
        "lam":       2e-5,                  # Регуляризация для FIRLS (Fast Iteratively Reweighted Least Squares)
        "alpha":     0.8,                   # Степень Lp-нормы для FIRLS (ближе к 1 = TV, ближе к 0.5 = hyper-Laplacian)
        "lambda_tv": 0.005,                 # Вес TV, если оркестратор выберет Ringing Removal
        "lambda_l0": 0.002,                 # Вес L0, если оркестратор выберет Ringing Removal
    },
    "auto_mode_params": {
        "poisson_denoiser": "vst_bm3d",     # Применение дисперсионно-стабилизирующего преобразования при пуассоновском шуме
    },
}
...
# hs_scale       : Масштаб распределения гиперболического секанса (b = 1/α). Меньшие значения усиливают разреженность градиентов (сильнее подавляют шум).
# noise_sigma    : Изначальная оценка стандартного отклонения шума (β_0 = 1/σ^2). Важнейший параметр для байесовского вывода.
# max_iter       : Максимальное количество внешних VB-EM итераций (обновление постериорных вероятностей) на один масштаб пирамиды.
# cg_iter        : Количество итераций метода сопряженных градиентов (CG) при оценке изображения (внутренний солвер).
# irw_iter       : Количество итераций IRLS (Iteratively Reweighted Least Squares) при не-слепом восстановлении.
# lambda_h_init  : Начальный вес L2-регуляризации (Тихонова) для оценки ядра в частотной области.
# lambda_h_decay : Фактор затухания регуляризации ядра (annealing). Уменьшает сглаживание ядра на поздних итерациях.

BID_HBSP_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Размер ядра ──
    "kernel_size":     {"type": "int",   "values": [21, 51]},

    # ── Параметры Байесовского вывода (VB-EM) ──
    "hs_scale":        {"type": "float", "values": [0.004, 0.02]},             # Масштаб HS-приора (b)
    "noise_sigma":     {"type": "float", "values": [0.002, 0.02]},             # Исходное предположение о шуме (σ)
    
    # ── Итерационные параметры солвера ──
    "max_iter":        {"type": "int",   "values": [20, 60]},                  # Внешние VB-EM итерации
    "cg_iter":         {"type": "int",   "values": [20, 80]},                  # Внутренние CG итерации (изображение)
    "irw_iter":        {"type": "int",   "values": [3, 10]},                   # Итерации IRLS (non-blind шаг)

    # ── Регуляризация ядра (в Фурье-пространстве) ──
    "lambda_h_init":   {"type": "float", "values": [10.0, 200.0]},             # Стартовый вес L2 для ядра
    "lambda_h_decay":  {"type": "float", "values": [0.85, 0.98]},              # Фактор экспоненциального затухания
}
