"""
Оптимизация гиперпараметров алгоритма VDBKE (Variational Deblurring with Known Edge Priors)
...
from blinddeconv.algorithms.blind_deconvolution.our_company.bayesian.vdbke_denoise.vdbke import VDBKE
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "9_vdbke_tpe_100iter_75dist"
...
# kernel_lambda      : Вес регуляризации распределения Дирихле для ядра. Задает уверенность в априорном распределении.
# img_lambda1        : Основной вес NGM-априорного распределения для изображения. Регулирует агрессивность сохранения краев.
# kernel_lower_bound : Параметр концентрации Дирихле. Предотвращает вырождение элементов ядра в абсолютный 0.
# firls_lambda / alpha: Параметры не-слепой FIRLS деконволюции. alpha ≈ 2/3 (аппроксимация сверхразреженной Lp-нормы).
# img_* / kernel_*_iter : Итерационные счетчики ADMM и Ньютоновских оптимизаторов внутри вариационного вывода.

VDBKE_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Размер ядра ──
    "kernel_size":        {"type": "categorical", "values": [15, 21, 25, 31, 35, 41, 51, 61]},

    # ── Регуляризация (вариационный вывод) ──
    "kernel_lambda":      {"type": "float", "values": [1e-8, 1e-3]},     # Вес ADMM/Dirichlet для ядра
    "img_lambda1":        {"type": "float", "values": [1e-4, 0.5]},      # Базовый вес TV/NGM априорного распределения изображения
    "img_lambda_min":     {"type": "float", "values": [1e-3, 0.1]},      # Мин. значение λ1 в мультимасштабном режиме
    "img_lambda_max":     {"type": "float", "values": [0.1, 10.0]},      # Макс. значение λ1
    "img_lambda_u":       {"type": "float", "values": [0.01, 1.0]},      # Множитель Лагранжа в ADMM
    
    # ── Оценка ядра (Dirichlet parameters) ──
    "gamma_correct":      {"type": "float", "values": [1.0, 2.2]},       # Гамма-коррекция
    "kernel_lower_bound": {"type": "float", "values": [0.1, 5.0]},       # Масса Дирихле априорного распределения (нижняя граница)
    "kernel_back_alpha":  {"type": "float", "values": [0.001, 0.1]},     # Шаг Backtracking (Ньютоновская оптимизация)
    "kernel_back_beta":   {"type": "float", "values": [0.3, 0.8]},       # Затухание Backtracking
    "kernel_lambda_C":    {"type": "float", "values": [0.0, 0.1]},       # Сглаживание градиентов ядра (Лапласиан)
    "kernel_threshold":   {"type": "float", "values": [0.0, 0.2]},       # Порог отсечения шума ядра
    "k_tol":              {"type": "float", "values": [1e-5, 1e-2]},     # Допуск сходимости ядра

    # ── Итерационные параметры ──
    "kernel_max_iter":    {"type": "int", "values": [5, 100]},           # Итерации оценки ядра на масштаб
    "img_N1":             {"type": "int", "values": [5, 80]},            # Внешние итерации оценки изображения
    "img_N2":             {"type": "int", "values": [1, 10]},            # Внутренние ADMM итерации
    "img_xv_iter":        {"type": "int", "values": [1, 5]},             # Итерации обновления переменных x-v
    "xk_iter":            {"type": "int", "values": [5, 50]},            # Чередования ядро-изображение

    # ── Параметры non-blind шага (FIRLS) ──
    "firls_lambda":       {"type": "float", "values": [1e-4, 0.5]},      # Вес регуляризации FIRLS
    "firls_alpha":        {"type": "float", "values": [0.3, 1.0]},       # Спектральная экспонента (Lp-норма)
    "firls_out_iter":     {"type": "int", "values": [2, 20]},            # Внешние итерации FIRLS
    "firls_inner_iter":   {"type": "int", "values": [2, 16]},            # Внутренние итерации FIRLS

    # ── Noise pipeline (вкл/выкл) ──
    "impulse_preprocess": {"type": "categorical", "values": ["none", "auto"]},
    "noise_estimation":   {"type": "categorical", "values": ["none", "pca", "chen"]},
    ...
    "blind_denoise":      {"type": "categorical", "values": ["none", "bilateral", "guided"]},
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    ...
    # Сбор параметров алгоритма VDBKE из конфигурации
    lr_params: Dict[str, Any] = {}
    for name in dict_config:
        lr_params[name] = config[name]

    # int-параметры
    _INT_KEYS = {"kernel_size", "kernel_max_iter", "img_N1", "img_N2",
                 "img_xv_iter", "xk_iter", "firls_out_iter", "firls_inner_iter"}
    for ik in _INT_KEYS:
        if ik in lr_params:
            lr_params[ik] = int(round(lr_params[ik]))

    # kernel_size: нечётный, преобразуем в tuple (ks, ks)
    ...
    lr_params["final_deconv"] = "firls"

    print(f"\n{'='*72}")
    print(f"  [eval {eval_id}] VDBKE params: {lr_params}")
    print(f"{'='*72}")
