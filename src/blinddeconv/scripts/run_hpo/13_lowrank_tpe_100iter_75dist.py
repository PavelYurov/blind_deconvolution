"""
Оптимизация гиперпараметров алгоритма LowRankBD (Low-Rank Blind Deconvolution)
...
from blinddeconv.algorithms.blind_deconvolution.our_company.low_rank_hyper_laplacian.lowrank_denoise.lowrank import LowRankBD
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "13_lowrank_tpe_100iter_75dist"
...
# lambda_           : Базовый вес гипер-лапласиановской регуляризации (α₀). Масштабируется по пирамиде.
# exp_a             : Экспонента гипер-лапласиана (p). Диапазон [0.4, 1.0] поощряет жесткие перепады яркости (сильные края).
# sigma             : Вес low-rank регуляризации ядра. Ключевой параметр алгоритма: принуждает ядро иметь низкий ранг (устраняет шум сложной структуры).
# tau / delta       : Параметры суррогатной оптимизации ядерной нормы (IRNN - Iteratively Reweighted Nuclear Norm).
# kernel_beta       : Вес Тихоновской регуляризации при CG-решении подзадачи ядра (предотвращает деление на 0).
# nb_lambda / alpha : Параметры быстрого гипер-лапласиановского не-слепого восстановления (MAP-вывод).

LOWRANK_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Размер ядра ──
    "kernel_size":      {"type": "categorical", "values": [15, 21, 25, 31, 35, 41, 51, 61]},

    # ── Основные параметры Hyper-Laplacian (Изображение) ──
    "lambda_":          {"type": "float",       "values": [1e-4, 1e-1]},       # Вес краевой регуляризации (α₀)
    "kernel_beta":      {"type": "float",       "values": [1e-4, 1e-1]},       # Вес Тихонова для ядра (β)
    "thr_e":            {"type": "float",       "values": [1e-5, 1e-2]},       # Сглаживание IRLS (ε)
    "exp_a":            {"type": "float",       "values": [0.4, 1.0]},         # Экспонента гипер-лапласиана (p)
    "alpha_multiplier": {"type": "float",       "values": [1.0, 4.0]},         # Шаг изменения α по пирамиде
    "threshold":        {"type": "float",       "values": [0.0, 0.15]},        # Порог отсечения шума ядра

    # ── Low-rank параметры ядра (IRNN) ──
    "sigma":            {"type": "float",       "values": [0.0, 5.0]},         # Сила low-rank регуляризации ядра
    "tau":              {"type": "float",       "values": [1e-7, 1e-3]},       # Проксимальный параметр сингулярных чисел
    "delta":            {"type": "float",       "values": [1e-7, 1e-3]},       # Сглаживание log-det суррогата

    # ── Итерационные параметры ──
    "max_iter":         {"type": "int",         "values": [3, 15]},            # Внешние альтернирующие итерации на масштаб
    "max_irls":         {"type": "int",         "values": [1, 5]},             # Внешние IRLS итерации (изображение)
    "max_cg":           {"type": "int",         "values": [50, 400]},          # Внутренние CG итерации (изображение)
    "max_iter_k":       {"type": "int",         "values": [20, 200]},          # CG итерации (ядро)
    "max_iter_rank":    {"type": "int",         "values": [1, 5]},             # Итерации IRNN (понижение ранга)
    "iter_k_rank":      {"type": "int",         "values": [1, 5]},             # Чередования ядро-ранг

    # ── Параметры non-blind шага (Hyper-Laplacian MAP) ──
    "nb_lambda":        {"type": "float",       "values": [100.0, 10000.0]},   # Вес fidelity для не-слепого шага
    "nb_alpha":         {"type": "float",       "values": [0.3, 1.0]},         # Экспонента приора не-слепого шага
    ...
    "blind_denoise":      {"type": "categorical", "values": ["none", "guided", "bilateral"]},
...
# --- Worker обработки отдельного изображения для ThreadPoolExecutor ---
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    ...
    # Сбор параметров алгоритма LowRankBD из конфигурации
    lr_params: Dict[str, Any] = {}
    for name in dict_config:
        lr_params[name] = config[name]

    # int-параметры: округляем и приводим к int
    _INT_KEYS = {"kernel_size", "max_iter", "max_irls", "max_cg",
                 "max_iter_k", "max_iter_rank", "iter_k_rank"}
    for ik in _INT_KEYS:
        if ik in lr_params:
    ...
    # lambda_ : в hp_space ключ "lambda_", конструктор тоже принимает lambda_
    # (без переименования — работает напрямую)

    # Применение фиксированных параметров алгоритма
    lr_params["nb_method"] = "hyper_laplacian"

    print(f"\n{'='*72}")
    print(f"  [eval {eval_id}] LowRankBD params: {lr_params}")
    print(f"{'='*72}")
