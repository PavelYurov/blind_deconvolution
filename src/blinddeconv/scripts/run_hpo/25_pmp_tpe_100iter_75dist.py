"""
Оптимизация гиперпараметров алгоритма PMP-BD (Patch-wise Minimum Pixels Prior)
...
from blinddeconv.algorithms.blind_deconvolution.our_company.patch_wise_minimum_pixels_prior.pmp_denoise.pmp import PMP_BD
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "25_pmp_tpe_100iter_75dist"
...
# --- Фиксированные параметры алгоритма PMP_BD ---
# Настройки конвейера предобработки и архитектуры метода, не подлежащие оптимизации.
_PMP_FIXED_PARAMS: Dict[str, Any] = {
    "gamma_correct":      1.0,               # Отключение гамма-коррекции перед оценкой ядра (identity mapping)
    "pmp_quantile":       0.0,               # Использование строгого минимума в патчах (квантиль 0.0) для PMP
    "ensemble_denoise":   True,              # Активация ансамблевой фильтрации (усредение guided-фильтров с разными радиусами)
    "impulse_preprocess": "auto",            # Адаптивный медианный фильтр для удаления импульсного шума
    "noise_estimation":   "pca",             # Оценка дисперсии шума методом анализа главных компонент (PCA)
    "act_preprocess":     "auto",            # Подавление шума в области кривлет-преобразования (ACT)
    "blind_denoise":      "guided",          # Использование guided-фильтра внутри слепого цикла
    "auto_mode":          "robust",          # Оркестратор для адаптации параметров в зависимости от уровня шума
    "final_deconv":       "ringing_removal", # Использование алгоритма подавления краевых артефактов
}


# --- Пространство поиска гиперпараметров (HP Space) ---
# Структура параметров базируется на алгоритме минимизации HQS (Half-Quadratic Splitting) 
# для совместно используемых априорных распределений L0-градиентов и локальных минимумов интенсивности (PMP).
PMP_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Параметры пространственной поддержки ──
    "kernel_size":      {"type": "int",   "values": [15, 65]},       # Размер ядра PSF (преобразуется в нечётный кортеж)

    # ── Весовые коэффициенты регуляризации ──
    "lambda_pmp":       {"type": "float", "values": [1e-3, 1.0]},    # Вес L0-регуляризатора интенсивности, основанного на локальных минимумах патчей (PMP)
    "lambda_grad":      {"type": "float", "values": [1e-4, 5e-2]},   # Вес L0-регуляризатора пространственных градиентов (μ в теоретическом описании)

    # ── Специфические параметры PMP (Patch-wise Minimal Pixels) ──
    "patch_r":          {"type": "int",   "values": [1, 7]},         # Радиус патча для поиска минимума интенсивности (размер окна: 2r+1 x 2r+1)

    # ── Управление итерационным процессом ──
    "xk_iter":          {"type": "int",   "values": [5, 30]},        # Количество итераций обновления связки "изображение-ядро" на каждом масштабе
    "k_thresh":         {"type": "float", "values": [5.0, 50.0]},    # Порог отсечения шума ядра на финальном масштабе (значения < max/k_thresh обнуляются)

    # ── Параметры фильтрации и сглаживания градиентов ──
    "denoise_eps":      {"type": "float", "values": [1e-4, 5e-2]},   # Параметр регуляризации (epsilon) для guided-фильтра при расчете градиентов
    "denoise_radius":   {"type": "int",   "values": [1, 5]},         # Базовый радиус guided-фильтра
    "grad_smooth_sigma":{"type": "float", "values": [0.0, 3.0]},     # СКО гауссова сглаживания градиентов (Bx, By) перед пороговой фильтрацией (0.0 = отключено)

    # ── Параметры неслепого шага восстановления (Ringing removal) ──
    "lambda_tv":        {"type": "float", "values": [1e-4, 1e-2]},   # Вес изотропной Total Variation (TV) регуляризации
    "lambda_l0":        {"type": "float", "values": [1e-4, 1e-2]},   # Вес разреживающего L0-градиентного регуляризатора
    "weight_ring":      {"type": "float", "values": [0.0, 2.0]},     # Весовой коэффициент вычитания артефактов звона
}
...
# --- Загрузка данных (75 изображений) ---
...
def _interpolate_pareto_surface(pareto_pts: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Интерполяция поверхности Парето-фронта с использованием RBF (thin-plate spline).
    Включает фильтрацию выбросов и сглаживание границ для формирования непрерывной поверхности.
    """
    xy = pareto_pts[:, :2]
    z  = pareto_pts[:, 2]
...
def _plot_pareto_3d_with_points_nd(
    ...
    """Построение 3D-графика Парето-фронта Noise Degradation (ND)."""
    fig = plt.figure(figsize=(18, 13))
    ax  = fig.add_subplot(111, projection="3d")
...
def _plot_pareto_surface_only_nd(
    ...
    """Построение исключительно интерполированной поверхности Парето-фронта Noise Degradation."""
    if len(pareto_pts) < 4:
        return
...
def _build_interactive_figure_nd(
    ...
    """Построение интерактивного Plotly 3D-графика Парето (НД вариант)."""
    fig = go.Figure()
    dominated = ~pareto_mask
    pareto_pts = all_pts[pareto_mask]
...
def update_pareto_plots(iteration: int) -> None:
    ...
    # Сохранение реестра точек для пост-анализа
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    """
    Вычисление целевой метрики для TPE оптимизатора.
    Метрика формируется как групповая медиана абсолютного значения SSIM восстановленных
    изображений (равные веса для чистых и зашумленных тестовых примеров).
    """
    _EVAL_COUNTER[0] += 1
    ...
    # Сбор параметров алгоритма PMP_BD из конфигурации
    lr_params: Dict[str, Any] = {}
    for name in dict_config:
        lr_params[name] = config[name]

    # int-параметры
    _INT_KEYS = {"kernel_size", "xk_iter", "patch_r", "denoise_radius"}
    for ik in _INT_KEYS:
        if ik in lr_params:
            lr_params[ik] = int(round(lr_params[ik]))

    # kernel_size должен быть нечётным
    if "kernel_size" in lr_params:
        ks = lr_params["kernel_size"]
        if ks % 2 == 0:
            ks += 1
        lr_params["kernel_size"] = max(ks, 3)

    # grad_smooth_sigma: 0.0 трактуем как отключение (None)
    if lr_params.get("grad_smooth_sigma", None) is not None \
            and float(lr_params["grad_smooth_sigma"]) <= 1e-6:
        lr_params["grad_smooth_sigma"] = None

    # Применение фиксированных параметров алгоритма
    lr_params.update(_PMP_FIXED_PARAMS)

    print(f"\n{'='*72}")
    print(f"  [eval {eval_id}] PMP_BD params: {lr_params}")
    print(f"{'='*72}")
    ...
    # Расчёт групповой медианы (обеспечивает равное влияние чистых и шумных тестов)
