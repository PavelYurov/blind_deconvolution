"""
Оптимизация гиперпараметров алгоритма LIP-BD (Condat-Vũ Primal-Dual) с использованием MCMC-FMP.
MCMC-FMP (MCMC — Factorized Mixture Proposal) — байесовский алгоритм поиска на основе марковских цепей Монте-Карло с факторизованным предложением смеси.
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "18_lip_cv_hmm_100iter_75dist"
...
# Остальные параметры подбираются оптимизатором (MCMC-FMP) или адаптивно внутри метода (robust).
...
def _plot_pareto_surface_only(
    ...
    """Построение только интерполированной поверхности Парето-фронта."""
    if len(pareto_pts) < 4:
        return
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    ...
    # Сбор параметров алгоритма из конфигурации MCMC-FMP
