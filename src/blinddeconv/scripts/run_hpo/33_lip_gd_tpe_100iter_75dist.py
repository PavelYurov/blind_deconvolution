"""
Оптимизация гиперпараметров алгоритма LIP-BD (Gradient Descent Primal-Dual)
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "33_lip_gd_tpe_100iter_75dist"
...
# Фиксированные параметры алгоритма LIP_BD (метод Gradient Descent / MM).
# Обоснование:
# method             : "mm" указывает на использование Majorization-Minimization
# auto_mode="robust" : Включает встроенный "оркестратор", который сам подбирает пространственные денойзеры в зависимости от уровня оцененного шума (σ).
# noise_estimation   : Использование PCA для точной оценки уровня шума перед началом деконволюции.
# nb_params          : Базовые веса для не-слепого шага (подавления звона), которые оркестратор будет масштабировать в зависимости от шума.
# auto_mode_params   : Тонкая настройка оркестратора (использование VST+BM3D для пуассоновского шума и ACT для гауссова).

_LIP_FIXED_PARAMS: Dict[str, Any] = {
    "method":             "mm",             # Режим градиентного спуска с мажоризацией
    "impulse_preprocess": "auto",           # Авто-удаление импульсного шума (salt & pepper)
    "final_deconv":       "auto",           # Авто-выбор метода финального восстановления (blend или ringing_removal)
    "auto_mode":          "robust",         # Включение робастного конвейера адаптации к шуму
    "noise_estimation":   "pca",            # PCA-метод для оценки дисперсии шума
    "nb_params": {
        "blend_weight": 0.5,                # Баланс между Tikhonov (резкость) и Ringing Removal (гладкость) для чистых изображений
        "lambda_tv":    0.005,              # Базовый вес TV-регуляризации для non-blind шага
        "lambda_l0":    0.002,              # Базовый вес L0-регуляризации для non-blind шага
    },
    "auto_mode_params": {
        "poisson_denoiser":          "vst_bm3d", # Использование Generalized Anscombe VST для пуассоновского шума
        "act_pre_nonblind_gaussian": True,       # Использование ACT (Curvelet) перед финальной деконволюцией
        "act_preprocess_gaussian":   True,       # Использование ACT (Curvelet) перед оценкой ядра
    },
}
...
# lambda_val (β) : Вес data-fidelity. Балансирует между резкостью (высокие значения) и подавлением шума.
# tau (ε)        : Параметр сглаживания нижней границы log-prior. Предотвращает деление на ноль и регулирует жесткость разреживания градиентов.
# outer_iters    : Количество внешних шагов MM (Majorization-Minimization) — пересчет мажорирующих весов.
# inner_iters    : Внутренние итерации градиентного спуска (GD) по изображению при фиксированных весах.
# k_step         : Шаг проекционного градиентного спуска для ядра.
# lambda_mult / scale_mult : Параметры coarse-to-fine пирамиды. Контролируют скорость убывания регуляризации и изменения масштаба.

LIP_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Размер ядра ──
    "kernel_shape":     {"type": "int",         "values": [21, 61]},

    # ── Core параметры blind deconvolution (градиентный спуск и MM) ──
    "lambda_val":       {"type": "float",       "values": [5e3, 1e5]},         # Вес fidelity (β)
    "tau":              {"type": "float",       "values": [5e-4, 5e-3]},       # Параметр нижней границы prior (ε)
    "outer_iters":      {"type": "int",         "values": [50, 150]},          # Внешние MM-итерации
    "inner_iters":      {"type": "int",         "values": [3, 15]},            # Внутренние шаги GD
    "k_step":           {"type": "float",       "values": [1e-3, 2e-2]},       # Шаг градиентного обновления ядра

    # ── Параметры пирамиды (Coarse-to-fine) ──
    "lambda_mult":      {"type": "float",       "values": [1.5, 3.0]},         # Множитель λ между уровнями
    "scale_mult":       {"type": "float",       "values": [1.2, 2.0]},         # Коэффициент масштабирования ядра

    # ── Постобработка ядра ──
    "kernel_threshold": {"type": "float",       "values": [0.01, 0.15]},       # Порог отсечения шума в PSF
}
...
def _interpolate_pareto_surface(pareto_pts: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # 1) NaN вне зоны покрытия точек (по расстоянию до ближайшего соседа)
    tree = cKDTree(xy)
...
def _plot_pareto_3d_with_points(
    ...
    """
    Построение 3D-графика: доминируемые точки, точки фронта и интерполированная поверхность.
    """
...
def _json_safe(v: Any) -> Any:
    """Рекурсивное приведение numpy / tuple / dict к JSON-сериализуемым типам."""
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    ...
    # kernel_shape — int → (n, n) tuple
    _ks = int(round(lr_params.pop("kernel_shape", 31)))
    lr_params["kernel_shape"] = (_ks, _ks)

    # k_step → массив из 1 элемента (одна фаза)
    if "k_step" in lr_params:
        lr_params["k_step"] = np.array([float(lr_params["k_step"])])

    # Применение фиксированных параметров алгоритма
    # Применение фиксированных параметров алгоритма
    # Применение фиксированных параметров алгоритма
    lr_params.update(_LIP_FIXED_PARAMS)
