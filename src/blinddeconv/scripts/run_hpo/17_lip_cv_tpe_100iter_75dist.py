"""
Оптимизация гиперпараметров алгоритма LIP-BD (Condat-Vũ Primal-Dual) с использованием TPE.
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "17_lip_cv_tpe_100iter_75dist"

# Углы обзора (elevation, azimuth) для сохранения проекций Парето-фронта
_VIEW_ANGLES = [
...
# Фиксированные параметры алгоритма LIP_BD (метод Condat-Vũ).
# Остальные параметры подбираются оптимизатором (TPE) или адаптивно внутри метода (robust).
...
# lambda_val (β) : Вес data-fidelity. Диапазон расширен до [5e3, 1e5] для охвата шумных и чистых сцен.
# tau (ε)        : Параметр нижней границы log-prior. Влияет на радиус проекции в dual-step.
# outer_iters    : Количество внешних итераций пересчета весов MM (Majorization-Minimization).
# inner_iters    : Количество шагов Condat-Vũ внутри каждой внешней итерации.
# k_step         : Скалярный шаг градиентного спуска по ядру (одна фаза).
LIP_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Размер ядра ──
    "kernel_shape":     {"type": "int",         "values": [21, 61]},

    # ── Core параметры blind deconvolution (ядро Condat-Vũ) ──
    "lambda_val":       {"type": "float",       "values": [5e3, 1e5]},         # Вес fidelity (β)
    "tau":              {"type": "float",       "values": [1e-4, 5e-3]},       # Параметр сглаживания prior (ε)
    "outer_iters":      {"type": "int",         "values": [50, 200]},          # Внешние MM-итерации
    "inner_iters":      {"type": "int",         "values": [3, 30]},            # Внутренние PD-шаги
    "k_step":           {"type": "float",       "values": [5e-4, 2e-2]},       # Шаг градиентного обновления ядра
    ...
    "lambda_tv":        {"type": "float",       "values": [1e-4, 1e-2]},       # Вес TV-регуляризации
    "lambda_l0":        {"type": "float",       "values": [1e-4, 1e-2]},       # Вес L0-градиента
    "weight_ring":      {"type": "float",       "values": [0.2, 1.2]},         # Сила подавления артефактов 
}
...
def _interpolate_pareto_surface(pareto_pts: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # RBF-интерполяция (thin-plate spline для гладкости)
    rbf = RBFInterpolator(xy, z, kernel='thin_plate_spline', smoothing=0.0)
    Zg = rbf(grid).reshape(Xg.shape)

    # 1) Мягкая обрезка по евклидову расстоянию (удаление артефактов экстраполяции)
    tree = cKDTree(xy)
    ...
    # 2) Исключение значений, выходящих за логические границы RBF интерполяции
    z_margin = 1.00 * (z_max - z_min) if z_max > z_min else 0.01
    Zg[(Zg < z_min - z_margin) | (Zg > z_max + z_margin)] = np.nan

    # 3) Очистка изолированных сегментов ("заплаток") - оставляем только крупнейшую часть
    valid = ~np.isnan(Zg)
    ...
    # 4) Лёгкое гауссово сглаживание поверхности (sigma=0.8)
    valid = ~np.isnan(Zg)
    if valid.any():
...
def _process_single_image(args: tuple) -> dict:
    ...
    # ---- восстановление ----
    psnr_rest, ssim_rest = psnr_dist, ssim_dist  
    error_msg = None
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    ...
    # Сбор параметров алгоритма из конфигурации TPE
    ...
    # Расчет групповой медианы (обеспечивает равное влияние чистых и шумных тестов)
