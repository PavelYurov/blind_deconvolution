"""
Оптимизация гиперпараметров алгоритма LMGP-BD (Laplacian Mixture Gaussian Prior)
...
from blinddeconv.algorithms.blind_deconvolution.our_company.lmgp.lmgp_denoise.lmgp import LMGP_BD
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "5_lmgp_tpe_100iter_75dist"
...
# lambda_lmg     : Вес уникального априорного распределения LMG (Local Maximum Gradient). Заставляет пиксели стремиться к локальному максимальному градиенту (сохранение текстур).
# lambda_grad    : Вес классического L0-градиентного априорного распределения (устранение мелкого шума, кусочно-постоянные области).
# xk_iter        : Количество альтернирующих шагов оценки (изображение <-> ядро) на одном масштабе пирамиды.
# k_thresh       : Обратный порог ядра (k[k < max/k_thresh] = 0). 
# denoise_eps / lmg_denoise_eps : Параметры управляемого фильтра (guided filter) для стабилизации градиентов перед их вычислением.
# lambda_tv / lambda_l0 / weight_ring : Параметры финальной не-слепой деконволюции (подавление звона).

LMGP_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Размер ядра ──
    "kernel_size":      {"type": "categorical", "values": [15, 21, 27, 31, 35, 41, 51, 61]},

    # ── Core параметры blind deconvolution (L0 и LMG) ──
    "lambda_lmg":       {"type": "float",       "values": [1e-4, 5e-2]},       # Вес регуляризатора локальных макс. градиентов (LMG)
    "lambda_grad":      {"type": "float",       "values": [1e-4, 5e-2]},       # Вес L0-градиентного регуляризатора
    "xk_iter":          {"type": "int",         "values": [2, 10]},            # Итерации (изображение-ядро) на масштаб
    "gamma_correct":    {"type": "float",       "values": [0.5, 2.0]},         # Степень гамма-коррекции
    "k_thresh":         {"type": "float",       "values": [5.0, 50.0]},        # Обратный порог отсечения ядра (max/k_thresh)
    "kernel_reg_weight":{"type": "float",       "values": [0.0, 0.1]},         # Tikhonov регуляризация ядра (стабильность к шуму)

    # ── Внутренние пространственные фильтры (стабилизация градиентов) ──
    "denoise_eps":      {"type": "float",       "values": [0.0, 0.05]},        # Сила денойзера перед извлечением градиентов ядра
    "denoise_type":     {"type": "categorical", "values": ["guided", "bilateral", "bm3d", "nlm"]},
    "lmg_denoise_eps":  {"type": "float",       "values": [0.0, 0.05]},        # Сила денойзера перед оператором LMG
    "lmg_denoise_type": {"type": "categorical", "values": ["guided", "bilateral", "bm3d", "nlm"]},
    ...
    "weight_ring":      {"type": "float",       "values": [0.0, 2.0]},         # Подавление артефактов (0 = выключено)
    ...
    "auto_params":        {"type": "categorical", "values": [0, 1]},           # Авто-подбор силы денойзеров по σ
}
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    ...
    # Сбор параметров алгоритма LMGP_BD из конфигурации
    ...
    # categorical bool: use_soft_threshold (0/1 → bool)
    if "use_soft_threshold" in lr_params:
        lr_params["use_soft_threshold"] = bool(int(lr_params["use_soft_threshold"]))

    # denoise_eps / lmg_denoise_eps: 0 → None (выключен)
    for eps_key in ("denoise_eps", "lmg_denoise_eps"):
        if eps_key in lr_params:
            val = float(lr_params[eps_key])
            lr_params[eps_key] = val if val > 1e-6 else None

    # grad_smooth_sigma: 0 → None (выключен)
    if "grad_smooth_sigma" in lr_params:
        val = float(lr_params["grad_smooth_sigma"])
        lr_params["grad_smooth_sigma"] = val if val > 0.05 else None

    lr_params["nonblind_method"] = "ringing_removal"

    print(f"\n{'='*72}")
    print(f"  [eval {eval_id}] LMGP_BD params: {lr_params}")
    print(f"{'='*72}")
