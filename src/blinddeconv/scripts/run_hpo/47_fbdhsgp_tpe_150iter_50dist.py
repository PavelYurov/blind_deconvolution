"""
Оптимизация гиперпараметров алгоритма FBDHSGP (Factorized Blind Deconvolution with Half-quadratic Sparse Gaussian Prior)
с использованием TPE.

Поиск оптимальных гиперпараметров методом TPE (Tree-structured Parzen Estimator)
...
Целевая функция TPE базируется на вычислении групповой медианы SSIM (с равным весом
...
from blinddeconv.algorithms.blind_deconvolution.our_company.bayesian.fbdhsgp.fbdhsgp_denoise.fbdhsgp import FBDHSGP
...
PARETO_OUTPUT_DIR = HPO_RL_ROOT / "47_fbdhsgp_tpe_150iter_50dist"
...
# Фиксированные параметры алгоритма FBDHSGP (не оптимизируемые).
# Обоснование:
# final_nb         : Строго зафиксировано использование FIRLS
# prior_name       : Использование логарифмического представления для Huber Super Gaussian Prior.
# beta_H / K2      : Константы ADMM. beta_H отвечает за жесткость равенства H = F P h. K2=1 означает один внутренний шаг ADMM.
# x_warm_start     : Включение передачи предыдущей оценки изображения (warm start) на следующий масштаб пирамиды.

_FBDHSGP_FIXED_PARAMS: Dict[str, Any] = {
    "gamma_correct":      1.0,              # Отключение гамма-коррекции
    "impulse_preprocess": "auto",           # Авто-удаление импульсных выбросов (защита для L2-fidelity)
    "auto_mode":          "robust",         # Адаптивное подключение BM3D/ACT при обнаружении шума
    "final_nb":           "frils",          # Использование FIRLS для финального восстановления (по умолчанию для FBDHSGP)
    "prior_name":         "Log",            # Использование логарифмического суррогата для вероятностного приора
    "beta_H":             10.0,             # Штрафной параметр ADMM для проекции ядра (h -> H)
    "K2":                 1,                # Количество внутренних итераций ADMM для переменной x
    "x_warm_start":       1,                # Использовать предыдущую оценку изображения как стартовую точку (warm start)
    "auto_mode_params": {
        "correlated_use_act_preblind": False, # Отключение тяжелого ACT-денойзера на этапе preblind (для скорости)
        "correlated_check":            False, # Отключение проверок на коррелированность шума (экономия времени)
    },
}

# --- Пространство поиска гиперпараметров (HP Space) ---
# Обоснование диапазонов:
# sigma       : Ожидаемое стандартное отклонение шума. Сильно влияет на баланс между data-fidelity и априорной моделью в ADMM.
# epsilon_min : Сглаживающий параметр для весов IRLS. Предотвращает деление на ноль при нулевых градиентах (Huber-like сглаживание).
# beta_v      : Штрафной параметр множителя Лагранжа в ADMM для переменных сплиттинга градиентов (v = ∇x).
# prior_alpha : Экспонента Lp-нормы (или Huber Super Gaussian). Ближе к 0.5 - сильные края (super-sparse), ближе к 1.0 - TV-подобное поведение.
# K1          : Количество внешних итераций обновления IRLS-весов (пересчет ковариации/весов).
# xh_iter     : Количество альтернирующих шагов оценки скрытого изображения и ядра.
# h_iter      : Внутренние итерации оценки ядра методом ADMM.
# delta / delta_x : Пороги сходимости по норме Фробениуса для ядра и изображения соответственно.

FBDHSGP_HP_SPACE: Dict[str, Dict[str, Any]] = {
    # ── Размер ядра ──
    "kernel_size":   {"type": "int",   "values": [21, 51]},

    # ── Параметры вероятностной модели (VB) ──
    "sigma":         {"type": "float", "values": [0.002, 0.03]},               # СКО шума для вероятностного вывода
    "prior_alpha":   {"type": "float", "values": [0.5, 1.2]},                  # Экспонента формы распределения (Lp-норма)

    # ── Параметры ADMM и IRLS ──
    "beta_v":        {"type": "float", "values": [0.05, 1.5]},                 # ADMM penalty для расщепления градиентов
    "epsilon_min":   {"type": "float", "values": [5e-4, 1e-2]},                # Сглаживание весов IRLS (Huber parameter)
    
    # ── Итерационные параметры ──
    "K1":            {"type": "int",   "values": [5, 20]},                     # Внешние итерации обновления весов (IRLS)
    "xh_iter":       {"type": "int",   "values": [5, 30]},                     # Чередования x-h на одном масштабе
    "h_iter":        {"type": "int",   "values": [5, 20]},                     # Внутренние ADMM итерации для оценки ядра

    # ── Критерии остановки (Tolerances) ──
    "delta":         {"type": "float", "values": [1e-3, 1e-2]},                # Допуск сходимости ядра
    "delta_x":       {"type": "float", "values": [5e-4, 5e-3]},                # Допуск сходимости изображения
}

# --- Структуры данных и глобальное состояние ---
@dataclass
class TestCase:
    """Один тест = (оригинал, искажённое, версия только-смаз)."""
    name: str
    original:  np.ndarray   # чистое изображение
    distorted: np.ndarray   # blur + noise
    blur_only: np.ndarray   # только blur (без шума)
    kernel_name: str
    noise_type:  str        # "" для изображений без шума
    has_noise:   bool


_PARETO_STORE: List[Dict[str, Any]] = []   # накопитель 3D-точек
_EVAL_COUNTER = [0]
...
def _parse_distorted_filename(
    ...
    """Разбирает «original_kernel_noise» → (orig, kernel, noise) или None."""
...
def _load_kernel(kernel_name: str) -> Optional[np.ndarray]:
    """Загружает ядро из ground_truth_filters/{name}_kernel.png."""
...
def _make_blur_only(original_f: np.ndarray, kernel: np.ndarray) -> np.ndarray:
    """Свёртка оригинала с ядром → версия только-смаз (float64 [0,1])."""
...
def load_test_cases() -> List[TestCase]:
    """Загружает все тестовые случаи из нового датасета."""
    ...
    print(f"Loaded {len(cases)} test cases, skipped {len(skipped)}: {skipped}")
    return cases
...
def _plot_pareto_3d_with_points(
    ...
    # Интерполяция поверхности Парето (зелёная, полупрозрачная)
    if len(pareto_pts) >= 4:
        try:
    ...
    for view_idx, (elev, azim) in enumerate(_VIEW_ANGLES, 1):
        ...
        print(f"  [Pareto] saved {path.name}")
    plt.close(fig)
...
def _plot_pareto_surface_only(
    ...
    """Только интерполированная поверхность (без точек) – яркая, зелёная."""
    if len(pareto_pts) < 4:
        return
    ...
    try:
        ...
        print(f"  Warning: surface-only plot failed: {e}")
        plt.close(fig)
        return
    ...
    for view_idx, (elev, azim) in enumerate(_VIEW_ANGLES, 1):
        ...
        plt.savefig(str(path), dpi=150, bbox_inches="tight", pad_inches=0.4)
        print(f"  [Pareto] saved surface-only: {path.name}")
    plt.close(fig)
...
def _plot_pareto_3d_with_points_nd(
    ...
    if len(pareto_pts) >= 4:
        try:
            ...
            print(f"  Warning: ND surface interpolation failed: {e}")
    ...
    for view_idx, (elev, azim) in enumerate(_VIEW_ANGLES, 1):
        ...
        plt.savefig(str(path), dpi=150, bbox_inches="tight", pad_inches=0.4)
        print(f"  [Pareto ND] saved {path.name}")
    plt.close(fig)
...
def _plot_pareto_surface_only_nd(
    ...
    try:
        ...
        print(f"  Warning: surface-only ND plot failed: {e}")
        plt.close(fig)
        return
    ...
    for view_idx, (elev, azim) in enumerate(_VIEW_ANGLES, 1):
        ...
        plt.savefig(str(path), dpi=150, bbox_inches="tight", pad_inches=0.4)
        print(f"  [Pareto ND] saved surface-only: {path.name}")
    plt.close(fig)
...
def _build_interactive_figure_nd(
    ...
    """Строит интерактивный 3D-график Парето (ND вариант, Plotly)."""
    fig = go.Figure()
    dominated = ~pareto_mask
    pareto_pts = all_pts[pareto_mask]
    ...
    if len(pareto_pts) >= 4:
        try:
            ...
            print(f"  Warning: ND interactive surface failed: {e}")
...
def _build_interactive_figure(
    ...
    """Строит интерактивный 3D-график Парето (Plotly) с переключением режимов."""
    fig = go.Figure()
    dominated = ~pareto_mask
    pareto_pts = all_pts[pareto_mask]
    ...
    if len(pareto_pts) >= 4:
        try:
            ...
            print(f"  Warning: interactive surface failed: {e}")
...
def update_pareto_plots(iteration: int) -> None:
    """Обновление Парето-графиков: строит и сохраняет два типа графиков для PSNR и SSIM."""
    ...
    # ── PNG → pareto_plots/PSNR, pareto_plots/SSIM ──
    ...
    # ── HTML → pareto_interactive/PSNR, pareto_interactive/SSIM ──
    if _HAS_PLOTLY:
        ...
        for metric, pts, mask in [("PSNR", psnr_pts, psnr_mask),
            ...
            print(f"  [Pareto] saved interactive: {html_path.name}")
    else:
        print("  [Pareto] plotly not installed — skipping interactive HTML")
    ...
    if _HAS_PLOTLY:
        ...
        for metric, pts, mask in [("PSNR", nd_psnr_pts, nd_psnr_mask),
            ...
            fig.write_html(str(html_path))
            print(f"  [Pareto ND] saved interactive: {html_path.name}")
...
def _save_pareto_json(path):
    records = []
    for d in _PARETO_STORE:
        rec = {k: _json_safe(v) for k, v in d.items() if k != "params"}
        rec["params"] = _json_safe(d["params"])
        records.append(rec)
...
def _process_single_image(args: tuple) -> dict:
    """Обрабатывает одно изображение: вычисляет PSNR/SSIM для blur, dist, restored."""
    ...
    try:
        ...
        if np.all(np.isfinite(restored.astype(np.float64))):
            ...
            # Штраф за слишком малую вариацию восстановленного изображения (порог /20)
            if std_restored is not None and std_distorted is not None:
                if std_restored < std_distorted / 20.0:
                    ssim_rest -= 0.4
            
            # Штраф за слишком острое ядро (нормализованная энтропия < 0.3)
...
def objective_function(config: Dict[str, Any], dict_config: Dict[str, Any]) -> float:
    """
    Метрика для TPE:  group-median(SSIM_restored)  по всем тестам.
    ...
    # ---- собираем параметры PMP_BD ----
    lr_params: Dict[str, Any] = {}
    for name in dict_config:
        lr_params[name] = config[name]

    # int-параметры
    _INT_KEYS = {"kernel_size", "K1", "xh_iter", "h_iter"}
    for ik in _INT_KEYS:
        if ik in lr_params:
            lr_params[ik] = int(round(lr_params[ik]))

    # kernel_size -> tuple
    if "kernel_size" in lr_params:
        ks = int(lr_params["kernel_size"])
        lr_params["kernel_size"] = (ks, ks)
    # ── Фиксированные настройки PMP_BD ──
    lr_params.update(_FBDHSGP_FIXED_PARAMS)

    print(f"\n{'='*72}")
    print(f"  [eval {eval_id}] FBDHSGP params: {lr_params}")
    print(f"{'='*72}")
...
config_TPE = {
    ...
    "backend": {
        ...
        "hp_space": FBDHSGP_HP_SPACE,
    },
}


if __name__ == "__main__":
    run_n_experiments(config_TPE, n_experiments=1, inference_only=False)

    # Финальный Парето-фронт (после всех итераций)
    if _PARETO_STORE:
