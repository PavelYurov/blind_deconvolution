...
- По ТИПАМ шума: ВСЕ-НА-ВСЕ (каждая пара изображение×ядро получает каждый тип).
- По УРОВНЯМ шума: round-robin (weak → medium → strong → weak → …).
...
def build_design_matrix(originals_list, kernels_list):
    ...
    Стратегия:
      - Чистый вариант (без шума): каждый (оригинал × ядро) — как раньше.
    ...
    (как было раньше), но теперь уровни шума варьируются между парами.
    ...
    """
    # Все пары (оригинал, ядро) — упорядоченные для воспроизводимости
    pairs = list(itertools.product(originals_list, kernels_list))

    design = []

    # 1) Чистый вариант (без шума)
    for orig, kern in pairs:
        design.append((orig, kern, "", "", None))

    # 2) Для каждого типа шума — round-robin по уровням
    for noise_type in NOISE_TYPES:
        levels = NOISE_LEVELS[noise_type]  # список из 3 dict'ов
        for idx, (orig, kern) in enumerate(pairs):
            lvl = levels[idx % len(levels)]  # round-robin
