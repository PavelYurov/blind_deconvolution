...
ALGORITHM_LABEL = "Heavy-Tailed Priors"
NUM_WORKERS = 6  # Количество процессов для параллельного подсчетов датасетов
...
ALG_MODULE = ("src.blinddeconv.algorithms.blind_deconvolution."
              "our_company.bayesian.htp.htp_cython._build_pyd.htp")
ALG_CLASS  = "HTP_BD"
...
ALG_KWARGS = {
    'kernel_size': 51,                   
    'noise_estimation': 'pyatykh',       
    'impulse_preprocess': 'auto',        
    'auto_mode': 'auto',                 
    'kernel_thresh': 0.05,               
    'lambda_tv': 0.005,                  
    'lambda_l0': 0.002,                  
    'impulse_params': {'density_threshold': 0.0005}
}
...
def run_phase1_datasets():
    ...
    for dataset_name in DATASETS_PHASE1:
        ...
        # Complexity_Test: несколько подсчет для стабильного времени
...
def _make_nonblind_func(alg_kwargs):
    ...
    from src.blinddeconv.algorithms.blind_deconvolution.our_company.\
        bayesian.htp.htp_denoise.non_blind import ringing_artifacts_removal

    lambda_tv = alg_kwargs.get('lambda_tv', 4e-3)
    lambda_l0 = alg_kwargs.get('lambda_l0', 2e-3)
    weight_ring = alg_kwargs.get('weight_ring', 0.5)
...
def run_phase2_iterations():
    ...
    for dist_file in distorted_files:
        ...
        kwargs['iteration_callback'] = logger
        alg = alg_cls(**kwargs)
        try:
...
GRID_PARAM_1 = "gamma"
GRID_VALUES_1 = [10.0, 30.0, 50.0, 100.0, 200.0, 500.0, 1000.0]

GRID_PARAM_2 = "Lp"
GRID_VALUES_2 = [0.1, 0.2, 0.3, 0.5, 0.7, 1.0]
