...
ALGORITHM_CONFIGS: list[dict[str, Any]] = [
    {
        ...
        "alg_module":       ("src.blinddeconv.algorithms.blind_deconvolution."
                             "our_company.logarithmic_pds.lip_cython._build_pyd.lip"),
        "alg_class":        "LIP_BD",
        "kernel_param":     "kernel_shape",
        "kernel_is_tuple":  True,
        "standard_kwargs": {
            # статья + модификации
            "kernel_shape":       (51, 51),
            "method":             "cv",
            ...
            "final_deconv":       "auto",
            "auto_mode":          "robust",
            "nb_params":          {"blend_weight": 0.5,
                                   "lambda_tv":    0.005,
                                   "lambda_l0":    0.002,
                                   "weight_ring":  1.0},
            "auto_mode_params":   {"poisson_denoiser":           "vst_bm3d",
                                   "act_pre_nonblind_gaussian":  True,
                                   "act_preprocess_gaussian":    True},
            "verbose":            False,
        },
        "pareto_tables_dir":
            PROJECT_ROOT / "presentation_graphics_pareto"
            / "pareto_3_lip_cv_tpe_150iter_50dist",
        "int_keys":     ["kernel_shape", "outer_iters", "inner_iters"],
        "odd_keys":     ["kernel_shape"],
        "tunable_keys": ["kernel_shape", "lambda_val", "tau",
                         "outer_iters", "inner_iters", "k_step",
                         "lambda_mult", "scale_mult", "kernel_threshold",
        ...
        "history_path": (
            PROJECT_ROOT / "presentation_graphics_pareto"
            / "pareto_3_lip_cv_tpe_150iter_50dist"
            / "logs" / "TPE" / "20260429_110402" / "history_0.csv"
        ),
    },
]
...
def process_one(task: dict) -> dict:
    ...
    nb_keys = task.get("nb_params_keys", [])
    nb_base = task.get("nb_params_base", {})
    if nb_keys:
        nb = dict(nb_base)
        if any(k in alg_kwargs for k in nb_keys):
            for k in nb_keys:
                if k in alg_kwargs:
                    nb[k] = alg_kwargs.pop(k)
            alg_kwargs["nb_params"] = nb
...
def _build_tasks_for_table(
    ...
    for idx, row in df.iterrows():
        ...
        common = {
            ...
            "row_meta":       row_meta,
            # Для LIP: плоские tunable-ключи, которые нужно упаковать в nb_params
            "nb_params_keys": algo.get("nb_params_keys", []),
            "nb_params_base": algo.get("nb_params_base", {}),
        }
