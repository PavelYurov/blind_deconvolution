...
RESULTS_ROOT  = PROJECT_ROOT / "presentation_graphics_denoise_with_blur"
...
def load_design() -> dict:
    ...
    with open(str(DESIGN_JSON), encoding="utf-8") as f:
        records = json.load(f)
    for r in records:
        if "psnr" not in r and "final_psnr" in r:
            r["psnr"] = r["final_psnr"]
        if "ssim" not in r and "final_ssim" in r:
            r["ssim"] = r["final_ssim"]
...
def _process_task(task: dict) -> dict:
    ...
    denoiser_params = {}
    if denoiser == "tv":
        denoiser_params = {"max_num_iter": 200}
    ...
    H, W = restored_u8.shape
    orig_cropped = original[:H, :W]

    orig_f = orig_cropped.astype(np.float64) / 255.0
