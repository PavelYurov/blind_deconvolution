...
ALGORITHM_LABEL = "Reweighted Graph TV"
NUM_WORKERS = 6  # Количество процессов для параллельного подсчета датасетов

# Пути
DATASETS_ROOT = PROJECT_ROOT / "images" / "compare_data" / "pasha"
...
ALG_MODULE = ("src.blinddeconv.algorithms.blind_deconvolution."
              "our_company.graph.gbbid_cython._build_pyd.gbbid")
ALG_CLASS  = "GBBID"
ALG_KERNEL_PARAM = "k_estimate_size"
...
ALG_KWARGS = {
    'k_estimate_size': 51,
    'border': 20,
    'nonblind_method': 'ringing_removal',
    'nonblind_params': {'lambda_tv': 0.005, 'lambda_l0': 0.002},
    'auto_mode': 'robust',
...
def _make_nonblind_func(alg_kwargs):
    ...
    from src.blinddeconv.algorithms.blind_deconvolution.our_company.\
        graph.gbbid_denoise.solvers import ringing_artifacts_removal
...
def run_phase3_hyperparam_grid():
    ...
    for dist_file in distorted_files:
        ...
        for v1, v2 in combos:
            ...
            kwargs = dict(ALG_KWARGS)
            kwargs[ALG_KERNEL_PARAM] = ks
            # GBBID: веса ringing_removal идут в nonblind_params, а не в конструктор.
            nb_params = dict(ALG_KWARGS.get('nonblind_params') or {})
            nb_params[GRID_PARAM_1] = v1
            nb_params[GRID_PARAM_2] = v2
            kwargs['nonblind_params'] = nb_params
            ...
            if original is not None:
                ...
                # aligned=True — shift-aligned PSNR/SSIM (компенсируется
