...
ALGORITHM_LABEL = "Fast BD Hyper-Sparse Gradient"
...
ALG_MODULE = ("src.blinddeconv.algorithms.blind_deconvolution."
              "our_company.bayesian.fbdhsgp.fbdhsgp_cython._build_pyd.fbdhsgp")
ALG_CLASS  = "FBDHSGP"
...
# ══════════════════════════════════════════════════════════════════════════════
ALG_KWARGS = {
    'kernel_size': (51, 51),        # будет нормализован под GT в _process_worker
    'auto_mode': 'robust',
    'impulse_preprocess': 'auto',
    'final_nb': 'frils',           
    'auto_mode_params': {
        'correlated_use_act_preblind': False,
        'correlated_check': False,
    },
    'kernel_threshold': 0.05,
    # 'prior_name': 'Log', #, , ,,,,'NL1''MOG''Lp'
}
...
def run_phase1_datasets():
    """Фаза 1: подсчет алгоритма на датасетах."""
    print("=" * 80)
    ...
    for dataset_name in DATASETS_PHASE1:
        ...
        print(f"\n[{dataset_name}] {len(distorted_files)} изображений (подсчет: {runs})")
...
def _make_nonblind_func(alg_kwargs):
    ...
    from src.blinddeconv.algorithms.blind_deconvolution.our_company.\
        bayesian.vdbke_denoise.solvers import ringing_artifacts_removal

    nb = alg_kwargs.get('nb_params') or {}
    lambda_tv = nb.get('lambda_tv', 1e-3)
    lambda_l0 = nb.get('lambda_l0', 5e-4)
    weight_ring = nb.get('weight_ring', 1.0)
...
def compute_fbdhsgp_pyramid(kernel_size, image_size):
    """Симулирует уровни пирамиды."""
    max_ks = max(kernel_size)
    ind1 = 0 if kernel_size[0] >= kernel_size[1] else 1
    ind2 = 1 - ind1

    minsize = [0, 0]
    minsize[ind1] = max(3, 2 * ((max_ks - 1) // 64) + 1)
    t2 = (kernel_size[ind2] * minsize[ind1]) // kernel_size[ind1]
    if t2 % 2 == 0:
        t2 += 1
    minsize[ind2] = max(t2, 3)

    resize_step = math.sqrt(2.0)
    ksize_list = []
    tmp = minsize[ind1]
    while tmp < max_ks:
        row = [0, 0]
        row[ind1] = int(tmp)
        t2 = int(math.ceil(kernel_size[ind2] / kernel_size[ind1] * tmp))
        if t2 % 2 == 0:
            t2 += 1
        row[ind2] = max(t2, 3)
        ksize_list.append(row)
        tmp = int(math.ceil(tmp * resize_step))
        if tmp % 2 == 0:
            tmp += 1
    ksize_list.append([int(kernel_size[0]), int(kernel_size[1])])

    pyramid_map = {}
    n = len(ksize_list)
    for i, (k1, k2) in enumerate(ksize_list):
        if i == n - 1:
            r, c = image_size
        else:
            r = int(math.floor(image_size[0] * k1 / kernel_size[0]))
            c = int(math.floor(image_size[1] * k2 / kernel_size[1]))
        pyramid_map[(k1, k2)] = (r, c)
    return pyramid_map
...
def run_phase2_iterations():
    ...
    from src.blinddeconv.algorithms.blind_deconvolution.our_company.\
        bayesian.fbdhsgp.fbdhsgp_denoise.utils import imresize_bilinear as _imresize
    print("\n" + "=" * 80)
    ...
    for dist_file in distorted_files:
        ...
        pyramid_map = compute_fbdhsgp_pyramid((ks, ks), blur_gray.shape[:2])
        pyramid_dir = img_save_dir / "restored_pyramid"
        pyramid_dir.mkdir(parents=True, exist_ok=True)
        _call_ctr = [0]
        _base_nb = nonblind_func

        def _pyramid_nonblind(blurred_full, kernel,
                              _ctr=_call_ctr, _dir=pyramid_dir,
                              _pmap=pyramid_map, _nb=_base_nb):
            _ctr[0] += 1
            kshape = kernel.shape[:2]
            full_h, full_w = blurred_full.shape[:2]
            target_r, target_c = _pmap.get(kshape, (full_h, full_w))
            if (target_r, target_c) != (full_h, full_w):
                blurred_s = _imresize(blurred_full, (target_r, target_c))
            else:
                blurred_s = blurred_full
            restored_s = np.clip(_nb(blurred_s, kernel), 0.0, 1.0)
            cv.imwrite(
                str(_dir / f"restored_k{kshape[0]}x{kshape[1]}_iter{_ctr[0]:04d}.png"),
                (restored_s * 255).astype(np.uint8),
            )
            if (target_r, target_c) != (full_h, full_w):
                return _imresize(restored_s, (full_h, full_w))
            return restored_s
...
GRID_PARAM_1 = "sigma"
GRID_VALUES_1 = [0.005, 0.008, 0.011, 0.014, 0.016, 0.018, 0.020]

GRID_PARAM_2 = "prior_alpha"
GRID_VALUES_2 = [0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2]
...
def run_phase3_hyperparam_grid():
    ...
    for dist_file in distorted_files:
        ...
        for v1, v2 in combos:
            ...
            kwargs = dict(ALG_KWARGS)
            kwargs[ALG_KERNEL_PARAM] = (ks,ks)
            # FBDHSGP: оба параметра идут напрямую в конструктор FBDHSGP.
