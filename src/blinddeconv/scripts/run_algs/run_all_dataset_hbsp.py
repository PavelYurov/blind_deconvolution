...
ALGORITHM_LABEL = "Hierarchical Bayesian SP"
...
# ══════════════════════════════════════════════════════════════════════════════
ALG_KWARGS = {
    'kernel_shape': (35, 35),       # будет нормализован под GT в _process_worker
...
def _make_nonblind_func(alg_kwargs):
    ...
    from src.blinddeconv.algorithms.blind_deconvolution.our_company.\
        bayesian.bid_hbsp_denoise.non_blind import ringing_artifacts_removal

    nb = alg_kwargs.get('nb_params') or {}
    lambda_tv = nb.get('lambda_tv', 4e-3)
    lambda_l0 = nb.get('lambda_l0', 2e-3)
...
GRID_PARAM_1 = "hs_scale"
GRID_VALUES_1 = [0.004, 0.006, 0.008, 0.010, 0.012, 0.014, 0.015]

GRID_PARAM_2 = "lambda_h_init"
GRID_VALUES_2 = [10.0, 25.0, 50.0, 80.0, 120.0, 160.0, 200.0]
