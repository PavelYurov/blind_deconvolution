...
Генерация датасета по принципу "ВСЕ-НА-ВСЕ":
Каждое оригинальное изображение обрабатывается каждым ядром размытия,
а затем к результату применяется каждый тип шума (включая вариант без шума).
...
INPUT_ORIGINALS_DIR = FRAMEWORK_ROOT / "images" / "compare_data" / "Levin" / "originals"
INPUT_KERNELS_DIR   = FRAMEWORK_ROOT / "images" / "compare_data" / "Levin" / "ground_truth_filters"

# 2. Куда сохраняем готовый датасет
OUTPUT_DATASET_DIR  = FRAMEWORK_ROOT / "images" / "compare_data" / "Levin"
...
def build_design_matrix(originals_list, kernels_list):
    ...
    каждый оригинал × каждое ядро × каждый шум (включая чистый вариант без шума)
    """
    # "" означает чистую картинку (без шума)
    all_noises = [""] + NOISE_TYPES
    
    # itertools.product создает все возможные комбинации элементов списков
    design = list(itertools.product(originals_list, kernels_list, all_noises))
    return design
...
def main():
    ...
    # Статистика
    n_total = len(design)
    n_clean = sum(1 for _, _, noise in design if not noise)
    n_noisy = n_total - n_clean

    print(f"  Оригиналов:  {len(originals_names)}")
    print(f"  Ядер:        {len(kernels_names)}")
    print(f"  Типов шумов: {len(NOISE_TYPES)} + 1 вариант без шума")
    ...
    for i, (orig, kernel, noise) in enumerate(design):
        ...
        # Имя файла: {orig}_{kernel}_{noise_type}.png 
        # Если шума нет, будет подпись "clean"
        ...
        print(f"  [{i+1:4d}/{n_total}] {out_name:50s}  "
