...
Облегчённая генерация датасета:
  10 изображений × 6 ядер × 4 варианта = 240 изображений.

4 варианта на каждую пару (изображение × ядро):
  1) clean  — только смаз, без шума
  2) weak   — слабый шум
  3) medium — средний шум
  4) strong — сильный шум
...
# Откуда берутся исходные файлы (только PNG)
UNIVERSAL_PART = "priors"; SIZE_IMAGE = 255*255
...
INPUT_ORIGINALS_DIR = FRAMEWORK_ROOT / "images" / "compare_data" / "kostya" / UNIVERSAL_PART / "originals"
INPUT_KERNELS_DIR   = FRAMEWORK_ROOT / "images" / "compare_data" / "kostya" / UNIVERSAL_PART / "ground_truth_filters"

# Куда сохраняем готовый датасет
OUTPUT_DATASET_DIR  = FRAMEWORK_ROOT / "images" / "compare_data" / "kostya" / UNIVERSAL_PART
