...
class Image:
    ...
    def get_len_filter(self):
        if(len(self.filters)!=len(self.blurred_array)):
            raise(Exception(f"filters and blurred images are not same amount {len(self.filters)} and {len(self.blurred_array)}"))
    ...
    def get_restored(self) -> str:
        return self.restored_paths.copy()              #.reshape(len(self.blurred_array)+1,len(self.algorithm))
    ...
    def get_PSNR(self) -> float:
        return self.psnr.copy()                #.reshape(len(self.blurred_array)+1,len(self.algorithm))

    def get_SSIM(self) -> float:
        return self.ssim.copy()                #.reshape(len(self.blurred_array)+1,len(self.algorithm))
    ...
    def add_algorithm(self, algorithm) -> None:  #название алгоритма, которым обрабатывалось
        self.algorithm = np.array(list(set(np.append(self.algorithm, algorithm)))) #теперь без повторов
