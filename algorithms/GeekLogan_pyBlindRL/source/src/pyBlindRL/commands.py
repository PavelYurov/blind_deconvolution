...
def RL_deconv(image, out, otf, iterations, target_device="cpu", eps=1e-10, approx=True):
    ...
    with torch.no_grad():
        ...
        image = torch.clone(image).detach().to(target_device, dtype=torch.complex64)

        out = torch.clone(out).detach().to(target_device, dtype=torch.complex64)

        otf = torch.clone(otf).detach().to(target_device, dtype=torch.complex64)
...
def RL_deconv_blind(gt_image, out_image, psf, iterations=20, rl_iter=10, eps=1e-9, reg_factor=0.01, target_device="cpu"):
    ...
    with torch.no_grad():
        tmp_image = gt_image.to(target_device, dtype=torch.complex64)
        out = torch.clone(out_image).detach().to(target_device, dtype=torch.complex64)
        out_psf = torch.clone(psf).detach().to(target_device, dtype=torch.complex64)
