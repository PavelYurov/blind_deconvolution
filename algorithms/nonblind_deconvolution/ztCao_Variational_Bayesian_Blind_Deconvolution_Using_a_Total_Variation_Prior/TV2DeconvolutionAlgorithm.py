...
class TV2DeconvolutionAlgorithm(DeconvolutionAlgorithm):
    ...
    def process(self, y: np.ndarray):
        ...
        h = self.psf.copy()  # Blur kernel
        laplacian = self.sparse_laplacian(y.shape)  # Laplacian regularizer
        ...
        for i in range(self.max_iter):
            ...
            # Step 1: Update x
            def A_func(img):
                psf_fft = self.precompute_psf_fft(h, y.shape)
                return self.apply_psf_fft_cached(img, psf_fft)

            x_new = self.solve_regularized_sparse(A_func, y, self.reg_param, laplacian)
            print(f"Step 1 completed.")

            # Step 2: Update h
            def blur_objective(h_flat):
                h_reshaped = h_flat.reshape(self.psf.shape)
                psf_fft_new = self.precompute_psf_fft(h_reshaped, y.shape)
                y_pred = self.apply_psf_fft_cached(x_new, psf_fft_new)
                data_fidelity = np.sum((y - y_pred) ** 2)
                grad_h = np.gradient(h_reshaped)
                smoothness = self.gamma * np.sum([np.sum(grad ** 2) for grad in grad_h])  # Correct smoothness term
            ...
            h = h_flat.reshape(self.psf.shape)
    ...
    def __init__(self, psf, max_iter=50, reg_param=1e-2, gamma=1e-2) -> None: 
        """
