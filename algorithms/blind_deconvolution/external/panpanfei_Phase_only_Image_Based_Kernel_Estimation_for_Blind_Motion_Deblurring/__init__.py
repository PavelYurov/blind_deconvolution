"""
PanpanfeiPhaseOnlyKernelEstimationBlindMotionDeblurring - Python Wrapper

Original Implementation:
    panpanfei (GitHub)
    GitHub Repository: https://github.com/panpanfei/Phase-only-Image-Based-Kernel-Estimation-for-Blind-Motion-Deblurring/
    Language/Framework: MATLAB

Reference Paper (if applicable):
    the image deblurring algorithm described in: [Phase-only image based kernel estimation for single image blind deblurring, CVPR2019.
...
Example:
    >>> from my_wrappers import PanpanfeiPhaseOnlyKernelEstimationBlindMotionDeblurring
    >>> processor = PanpanfeiPhaseOnlyKernelEstimationBlindMotionDeblurring()
    >>> result = processor.process(input_image)

Important Notes:
    1. Requires: numpy, opencv-python (cv2)
    2. Original MATLAB code remains unchanged
