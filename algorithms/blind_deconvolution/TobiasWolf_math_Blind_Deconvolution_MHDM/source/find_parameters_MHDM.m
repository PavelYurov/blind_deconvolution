...
function[lambda_0,mu_0,u_end,k_end,u_four_MHDM,k_four_MHDM,its] = find_parameters_MHDM(u_true,f,f_four,r,s,tol,stopping,maxits,minits,indices)
% finds optimal initial parameters for blind-dconvolution MHDM and returns
% the parameters and reconstructions obtained

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%Input: true image u_true, blurred and noisy image f and its Fourier
%transform f_four, powers r and s for sobolev norms in Fourier space, tolerance tol for small negative values due to numerical issues,
%stopping threshold stopping for MHDM, maximum number of MHDM iterations
%maxits, minimum numbers of MHDM iterations minits, indices for which anti-symmetry condition is enforced
%Output: initial MHDM parameters lambda_0 and mu_0, final iterates u_enda and k_end, Fourier transforms u_four and k_four of the reconstructed kernel and image
...
%compute middle psnr
ratio_mid = ratio_0;
[val_mid,dict] = check_ratio_MHDM(u_true,f,f_four,ratio_mid,r,s,tol,stopping,maxits,minits,indices,dict);

%compute left psnr
ratio_left = ratio_0/2;
[val_left,dict] = check_ratio_MHDM(u_true,f,f_four,ratio_left,r,s,tol,stopping,maxits,minits,indices,dict);

%compute right psnr
ratio_right = ratio_0*2;
[val_right,dict] = check_ratio_MHDM(u_true,f,f_four,ratio_right,r,s,tol,stopping,maxits,minits,indices,dict);
