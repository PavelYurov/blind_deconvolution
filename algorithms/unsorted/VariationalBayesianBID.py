...
class VariationalBayesianBIDAlgorithm(DeconvolutionAlgorithm):
	...
	def process(self, image: np.ndarray) -> np.ndarray:
		...
		if image.ndim == 2:
			...
			return cv2.merge([b_r, g_r, r_r]),KERNEL_PLACEHOLDER
		else:
	...
	def process(self, image: np.ndarray) -> np.ndarray:
		...
		if image.ndim == 3 and image.shape[2] == 3:
			...
			for ch in cv2.split(image):
				...
				out = np.clip(x*255.0, 0, 255).astype(np.uint8),KERNEL_PLACEHOLDER
				out_channels.append(out)
			return cv2.merge(out_channels)
		elif image.ndim == 2:
			y = image.astype(np.float64)/255.0
			x = self._process_gray(y)
			return np.clip(x*255.0, 0, 255).astype(np.uint8),KERNEL_PLACEHOLDER
		else:
